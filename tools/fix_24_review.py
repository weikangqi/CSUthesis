...
print(f"Section 2.4: lines {start_idx+1}–{end_idx}")
...
new_section = (
    ...
    r'本小节从结构轻量化设计、模型压缩与端侧部署工程优化三个层次依次介绍在边缘端实现高效推理的常用方法，'
    r'表~\ref{tab:lightweight_overview}给出了本文各层次的具体选择。' + '\n'
    '\n'
    r'\begin{table}[htbp]' + '\n'
    r'\centering' + '\n'
    r'\caption{轻量化设计各层次效率瓶颈、应对策略与本文采用方案}' + '\n'
    r'\label{tab:lightweight_overview}' + '\n'
    r'\small' + '\n'
    r'\begin{tabular}{@{}lll@{}}' + '\n'
    r'\toprule' + '\n'
    r'效率瓶颈 & 应对策略 & 本文采用 \\ \midrule' + '\n'
    r'骨干算术计算量大 & 深度可分离卷积、倒残差 & CSPDarknet 非对称骨干（宽度 M=1.0\,/\,N=0.5） \\' + '\n'
    r'双目左右分支冗余 & 非对称主/辅分支 & 左主分支（M）$+$右辅分支（N） \\' + '\n'
    r'高维代价体访存压力 & 极线方向特征聚合 & EFEM（1/16 分辨率，视差范围 $D=24$） \\' + '\n'
    r'存储与带宽开销 & 混合精度量化 & 骨干 INT8\,$+$\,注意力/热图头 FP16 \\' + '\n'
    r'\bottomrule' + '\n'
    r'\end{tabular}' + '\n'
    r'\end{table}' + '\n'
    '\n'
    r'\subsubsection{结构轻量化设计}' + '\n'
    ...
    r'轻量化设计中应重点控制通道宽度与融合路径数量，避免引入冗余分支。'
    r'双目结构天然存在"左右分支"带来的参数与算力倍增风险；'
    r'实践中常采用非对称主/辅分支——主分支（左视图）保留较大容量以提取充分特征，'
    r'辅分支（右视图）采用更轻量结构，在保证跨视图信息完整性的同时降低整体计算开销。' + '\n'
    '\n'
    r'端侧系统通常难以承受高分辨率、全视差范围的代价体与3D卷积正则化，'
    r'更可行的替代思路是在低分辨率特征上构建紧凑的跨视图交互，'
    r'以分组相关（Group-wise Correlation）等轻量算子代替全通道代价体，从而降低访存压力；'
    r'结合立体校正先验，可进一步将信息聚合限制在极线方向，将二维搜索退化为一维扫描——'
    r'这也是第三章极线特征增强模块（EFEM）的设计出发点。' + '\n'
    '\n'
    ...
    r'本文采用热力图引导的2D关节定位与可微加权三角化（DWT）相结合的方式，'
    r'在左相机坐标系下直接回归绝对3D关节坐标，兼顾定位精度与端侧算子友好性。' + '\n'
    '\n'
    r'\subsubsection{模型压缩}' + '\n'
    r'在高效结构的基础上，模型压缩进一步降低存储与计算开销。'
    r'实践中通常优先采用量化（对端侧吞吐提升最直接），再以剪枝缩减算量，最后以知识蒸馏补偿精度损失；'
    r'三者可单独或组合部署。' + '\n'
    '\n'
