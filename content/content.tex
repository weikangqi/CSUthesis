...
尽管基于深度学习的二维人体姿态估计已经取得显著进展\citess{zhang2022pose2dcn}，但二维结果本质上仍缺乏绝对深度与前后关系信息，在遮挡、复杂背景和多人交互场景下难以稳定描述人体的真实空间结构。进一步从单目图像恢复三维姿态时，又会面临由投影成像引入的深度歧义：不同三维骨架可能对应相近的二维观测，这使得单目三维姿态估计在绝对尺度恢复、遮挡关节推断和空间关系判断方面始终存在先天困难\citess{yu2024posecodeccn}。遮挡场景下的姿态估计研究也进一步说明，观测缺失会放大这种由二维到三维恢复带来的不确定性\citess{xu2024occlusioncn}。
...
然而，双目几何优势并不直接等价于端侧可用。现有高精度三维姿态估计与学习式立体匹配方法往往依赖重型骨干网络、高维代价体和多轮特征聚合，带来较大的参数量、FLOPs、显存访问与推理时延开销\citess{chang2018psmnet}。与此同时，移动终端和边缘设备还受到功耗、存储、带宽、持续运行稳定性以及隐私保护等多重约束，使“高精度”与“可部署”之间形成明显张力\citess{yuan2024aiotinfercn}。
...
贾迪等提出融合多关节特征的单目视觉三维人体姿态估计网络\citess{jia2025monocular3dhpecn}，通过多关节特征融合增强关节间依赖建模，在单目三维人体姿态估计中提高了准确性与泛化能力。

杨红红等提出基于平行多尺度时空图卷积网络的三维人体姿态估计算法\citess{yang2025pmstgnetcn}，构造对角占优的时空注意力图卷积和多尺度交叉融合模块，以增强骨架关节点的时空关系建模能力。
...
董婧等提出联合多视图可控融合和关节相关性的三维人体姿态估计方法\citess{dong2025cfjcnetcn}，通过多视图可控融合优化模块、关节相关性建模和结构化三角剖分联合提升二维热图质量与三维重建精度。

Zhang 等人提出 Direct Multi-view Multi-person 3D Pose Estimation\citess{zhang2021direct}，尝试直接从多视图观测中恢复多人三维姿态与实例关联，减少显式三维体素化带来的中间表示负担。
...
李曈等提出适应立体匹配任务的端到端深度网络\citess{li2020stereoadaptivecn}，针对特征提取冗余和 3D 卷积感受野受限问题设计适配立体匹配任务的特征提取结构与分离 3D 卷积，在保证精度的同时显著减少参数量与训练开销。

贾迪等提出面向弱纹理目标立体匹配的 Transformer 网络\citess{jia2024weaktexturestereocn}，通过空间池化窗口、重叠式块嵌入和跳跃查询策略增强弱纹理区域的上下文表示与遮挡估计能力。
...
值得注意的是，目前专门针对双目相机的端到端三维人体姿态估计研究仍较为匮乏：现有工作或沿用全稠密视差估计的多阶段级联思路，或直接将多视角方法应用于双目设置，鲜有方法从关节任务需求出发重新设计双目特征交互与几何恢复路径。近年来，随着轻量化骨干与极线约束注意力机制的逐步成熟，部分 2025 年的探索性工作已开始将极线几何约束嵌入特征交互模块以替代全图视差估计，但系统性的端到端双目三维姿态估计方法与面向端侧的量化部署研究仍极为稀缺。这一研究空白在端侧轻量化部署场景中尤为突出——级联方案的计算冗余与误差传播问题在资源受限设备上被进一步放大，表明设计面向端侧的端到端双目三维姿态估计框架具有重要的实际价值。
...
3）mAP：在 COCO 等检测式评测中，通常基于不同 OKS 阈值统计平均精度（mAP），用于综合评估多人场景下关键点检测与实例关联的整体表现\citess{lin2014microsoft}。
...
然而，双目约束并不天然等价于端侧友好。现有许多双目深度或三维重建方法仍依赖高维代价体构建与密集正则化，往往需要三维卷积或大规模迭代优化才能获得较高精度，这会带来显存占用高、访存压力大与推理延迟不可控等问题\citess{chang2018psmnet}。面向移动平台的立体匹配研究则进一步表明，为获得更可控的部署成本，必须主动压缩跨视图交互与代价聚合链路\citess{shamsafar2022mobilestereonet}。另一方面，若直接采用左右对称的双分支结构分别完成特征提取与姿态预测，骨干网络与多尺度融合模块的计算量将近似倍增，导致大量跨视图冗余计算；在多人场景中，该冗余会随实例数增加而放大，进一步削弱端侧实时性，并使系统时延随场景复杂度产生明显波动。
...
1）单目方法（仅左视图$I_l$）：选取可复现的代表性单目三维姿态估计网络作为基线，包括 SimpleBaseline\citess{martinez2017simple}、JointFormer\citess{lutz2022jointformer} 与 RTMW3D\citess{jiang2024rtmw}。


2）基于稠密深度先验的级联范式（双目输入$(I_l,I_r)$）：该范式与 3.2.3.1 节所述两阶段流程一致，先通过双目匹配估计稠密深度/视差，再与二维关键点结合进行三维回代/融合得到三维姿态。代表方法包括 RTMO\citess{lu2024rtmo}+SGBM（传统立体匹配）、RTMO\citess{lu2024rtmo}+MobileStereoNet\citess{shamsafar2022mobilestereonet}（学习式立体网络）以及 MobiDepthPose\citess{zhang2022mobidepth}。
...
对称全量方案取得最低 MPJPE（154.5\,mm），但 FLOPs 较本文非对称方案高出约 62\%；对称轻量方案虽然更紧凑，却带来明显精度退化。补充的左右交换实验表明，将主辅视角角色互换（辅助视角全量+主视角轻量）后，MPJPE 为 159.8\,mm，仅较本文配置高出 1.2\,mm，基本可视为无显著差别，说明非对称分工的有效性并不强烈依赖于哪一侧担任主分支；这一可互换性与第~\ref{sec:exp_setup}节所采用的水平翻转并同步交换左/右视图的数据增强策略相吻合——该策略在训练过程中持续打破固定视角分工，使模型更关注稳定的双目几何对应关系，从而降低对具体主辅侧位的结构性偏置。本文最终选择主视角全量、辅助视角轻量的非对称配置，在仅增加 4.1\,mm 误差的前提下显著压缩参数量与 FLOPs；其与实际端侧时延之间的对应关系将在第四章进一步分析。
...
\subsubsection{平台迁移讨论}
不同端侧后端对同一模型的友好部分并不相同，这也是本文不直接将 Snapdragon 8 Gen3 CPU 上得到的精度分配结论简单外推到 GPU/NPU/DSP 的原因。

对于 CPU 后端而言，主体骨干卷积既是最大时延来源，也是 ARMv8.2 dotprod 最容易兑现 INT8 吞吐收益的模块，因此成为量化主收益来源；与此同时，EFEM gather/correlation 与 DWT 几何求解虽然计算规模不大，却包含不规则访存、小矩阵求解和 softmax/反量化等对数值稳定性敏感的环节，CPU 对这些链路具有更高的执行完整性与调试可控性。

若迁移到 GPU 后端，规则卷积与矩阵乘法的吞吐通常更高，但小 batch 条件下的 kernel launch、跨设备张量搬运以及混合精度切换开销会被放大，原本在 CPU 上不显著的张量布局转换和 dequant/requant 代价可能成为新瓶颈。若迁移到 NPU 后端，低比特卷积与融合算子块往往具有更高能效，但 EFEM 的不规则 gather、softmax 前后分支以及 DWT 这样的显式几何求解通常难以完整映射到算子白名单之中，容易形成“主体在 NPU、关键链路回退 CPU/GPU”的异构碎片化执行。对于 DSP 后端，定点运算与低功耗优势更明显，但片上存储受限和开发链路碎片化会提高复杂视觉模型的接入成本。

因此，本文提出的方法论价值不在于某一组固定精度配置本身，而在于给出一条可迁移的分析路径：先用实测延迟和 Roofline 判断真正的热点模块，再用孤立量化敏感性筛查识别高风险层，最后结合目标后端的算子支持能力决定哪些模块进入 INT8 主路径、哪些模块保留高精度。也正因如此，当模型迁移到 iOS/Core ML、Android GPU/NPU 或其他 SoC 上时，需重新执行这一三步分析，而不能直接复用本文在 CPU + MNN 环境中得到的保护层集合。
