...
从端侧部署角度看，移动端 CPU 与边缘设备通常同时受到算力、内存带宽、缓存容量与功耗预算的约束，模型能否落地不仅取决于参数规模，还取决于权重访存模式、算子调度方式以及中间激活的存取开销。对于包含多尺度卷积、跨视图匹配与几何求解的三维姿态估计模型而言，这些约束往往相互耦合：即使网络结构本身已经完成轻量化，若关键模块仍存在高带宽消耗或数值敏感环节，端侧时延与精度退化问题仍难以同时得到解决。

现有端侧优化研究通常沿着轻量化网络结构、算子融合、低比特量化与混合精度部署等方向展开；对于移动端姿态估计与同类视觉模型，实际可落地方案往往不是单独依赖某一种压缩手段，而是将结构裁剪、算子优化与量化部署协同设计。经典整数推理工作通常采用逐通道权重量化与离线校准来兼顾吞吐与精度\cite{jacob2018quantization}；面向快速部署的 PTQ 方法则强调通过权重均衡、偏置修正与截断阈值搜索减小量化误差\cite{banner2019post,nagel2019data}; 而近年来的激活平滑方法进一步表明，长尾激活分布的处理方式会显著影响低比特部署效果\cite{xiao2023smoothquant}。这也说明，端侧量化并非“全模型一刀切”即可达到最优效果，精度风险往往集中在激活分布不稳定层、归一化与匹配相关环节，以及直接参与几何恢复的求解环节。对于本文模型而言，卷积主干、跨视图匹配、softmax 与几何求解并存，更适合采用“瓶颈分析 + 敏感性筛查 + 分层混合精度”的部署思路；在具体校准实现上，本文仅借鉴 NCNN 的直方图校准思路作为一种稳健 PTQ 方法。因此，第四章需要先识别端侧瓶颈，再确定哪些层适合量化、哪些层必须保护。

\subsection{问题分析}

在正式介绍量化方案之前，本节围绕“哪些模块最值得量化、哪些模块不能轻易量化”这一核心问题，按三个层次递进展开分析。首先，通过实测延迟拆解识别端侧推理的主要耗时来源，回答“优化重点在哪里”；其次，借助 Roofline 模型进一步区分主要模块受算力还是受带宽约束，回答“这些热点为何成为瓶颈”；最后，通过量化敏感性分析识别几何推理链路中的高风险层，回答“哪些层可以量化、哪些层必须保护”。这样，4.2 节将从性能热点、硬件约束与误差风险三个维度，为后续 4.3 节的分层混合精度设计建立完整证据链。

\subsubsection{端侧推理瓶颈的实测分析}

4.2.1 首先回答“端侧推理时间主要耗在哪些模块上”。为定位第三章所提框架在端侧的计算热点，本节以 Xiaomi 14（Snapdragon 8 Gen3，16\,GB RAM）为目标平台，采用 MNN 推理引擎在 FP16 精度、ARMv8.2 指令集、4 线程 CPU 模式下对各模块逐一计时。计时方法与第三章实验协议保持一致：输入分辨率为 $640\times480$，批大小为 1，测试前进行若干次预热以消除冷启动影响，正式计时取 100 次前向的均值，计时范围包含各模块的前向计算，不含图像解码与 I/O 开销，并以单人场景（$M=1$）为基准。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.68\textwidth]{images/ModuleDelayRatioFanChart}
  \caption{各模块端到端延迟占比}
  \label{fig:module_delay_pie}
\end{figure}

由表~\ref{tab:module_latency} 与图~\ref{fig:module_delay_pie} 可知，骨干网络是延迟的绝对主体；进一步地，图~\ref{fig:cspdarknet_stage_pie} 给出骨干内部各阶段的延迟分布，以定位量化的最优优先级。
...
在确认骨干网络是主要热点之后，还需要进一步回答“骨干内部哪些部分最值得优先量化”。从结构上看，CSPDarknet 对 INT8 量化具有天然适应性（网络结构详见第三章图~\ref{fig:cspdarknet_arch}），体现在三个方面。第一，Conv-BN 可融合：骨干中所有卷积均为标准 Conv+BN+SiLU 结构，MNN 图优化可自动将 BN 参数吸收至前置 Conv 权重，量化对象简化为单一权重矩阵，per-channel 对称量化可精确建模各输出通道的幅值分布。第二，激活分布稳定：SiLU 输出动态范围有界，权重幅值在 BN 归一化后集中，per-channel 缩放因子覆盖率高、量化误差小。第三，延迟由算术负载主导：非对称双分支（左 width=1.0，右 width=0.5）实测延迟比约 3.95$\times$（76.7ms vs.\,19.4ms），与理论计算量比吻合，说明骨干延迟由卷积算术负载决定，INT8 dotprod 加速可直接转化为延迟收益。

图~\ref{fig:cspdarknet_stage_pie} 进一步给出双分支各阶段延迟占比。P3/P4 阶段合计约占各分支延迟 70\%（分辨率适中、通道数大，FLOPs 密度最高），是 INT8 量化加速的主要来源；Stem 通道少延迟低；P5 虽通道最多，但分辨率最小（$15\times20$），FLOPs 绝对值反而低于 P4。
...
在 4.2.1 已经确认“骨干是主热点、EFEM 是次级热点”之后，4.2.2 进一步回答“这些热点为何形成瓶颈，以及量化收益主要来自哪里”。为此，引入 Roofline 模型以算术强度（Arithmetic Intensity, AI）刻画算子“算得多还是搬得多”，其中
...
之所以在 Roofline 中采用 $B_{\text{eff}}$ 而非 $B_{\text{peak}}$，关键原因在于推理阶段存在明显的 LLC 容量瓶颈：FP16 基线权重约 23.4\,MB，显著超过 LLC（8\,MB），骨干权重难以整体驻留片上，前向过程中需要按层从 LPDDR5X 流水加载，导致 LLC 缺失频繁。表~\ref{tab:memory_hierarchy} 给出存储层次的容量与带宽量级（L1/L2 为每核独享，LLC 为四核共享）。
...
在上述硬件参数设定下，进一步估算各模块在 FP16、batch=1 条件下的 AI。对于 $3\times3$ 卷积（输入通道 $C_{\text{in}}$、输出通道 $C_{\text{out}}$、分辨率 $H\!\times\! W$），有
\[
  \mathrm{AI}_{3\times3}
...
经过 4.2.1 与 4.2.2 的分析，已经知道“量化收益主要应来自骨干卷积，EFEM 中部分算子收益受带宽约束限制”。但这仍不足以直接给出量化范围，因为端到端三维姿态估计同时包含相关性匹配、softmax 归一化与几何求解等数值敏感环节，若盲目对全部层施加 INT8 可能引入不可接受的几何误差。因此，4.2.3 最后回答“哪些热点可以量化、哪些关键环节必须保护”，通过量化前敏感性筛查识别“可安全量化”的主体层与“必须保护”的高敏感模块，为后续分层混合精度方案提供依据。

为使这一筛查结论可直接服务于后续精度分配，本节以第三章 FP16 模型为基线，在 RT-Pose 验证集上以 MPJPE 均值作为精度指标。采用逐组替换的方式：每次仅将某一模块的权重或激活替换为 INT8 表示（per-channel 对称量化），其余模块保持 FP16，记录相对基线的变化量 $\Delta\text{MPJPE}$。其中权重量化对应卷积权重的静态离散化；激活量化对应中间特征图的动态离散化，两者对数值稳定性的影响路径不同。
...
从运算特性看，高敏感模块均包含“差异放大”的数值环节：EFEM 注意力分数进入 softmax 后，细微分数差异将影响匹配权重分布；DWT 依赖小规模矩阵分解/求解，离散化误差容易直接传递到三维坐标；热力图与置信度输出则决定峰值定位与可信度筛选，对输出分辨率与峰形更敏感。综合敏感性结果与 MNN CPU 推理对混合精度的实现方式（量化豁免层以 FP32 执行），由表~\ref{tab:quant_sensitivity} 可得到以下结论：
...
综合 4.2.1 的延迟拆解、4.2.2 的 Roofline 瓶颈判别与 4.2.3 的敏感性筛查结果，本节将“瓶颈位置”和“误差风险”统一到同一设计框架下。与仅直接给出量化配置不同，本文在本章中提出一套面向目标平台的三步式决策框架：先通过延迟拆解与 Roofline 定位性能瓶颈，再通过量化敏感性分析识别风险层，最后据此设计分层混合精度与 PTQ 校准流程。

设计目标可概括为：在保持几何推理稳定性的前提下，最大化主体卷积层的 INT8 覆盖率，使量化收益尽可能来自真正主导时延的模块，而不是来自边缘算子的局部压缩。

\subsubsection{分层精度分配方案}

4.3.1 的目标是把 4.2 的分析结果落到可执行的精度分配上。依据“性能贡献 × 数值风险”的联合标准，骨干与 EFEM 的主体卷积层既主导端侧时延，又表现出较低量化敏感性，因此作为 INT8 的主要覆盖对象；相反，EFEM 注意力分数、热力图/置信度输出以及 DWT 求解器虽然参数占比很小，却直接影响匹配分布、峰值判别和三维求解稳定性，因此统一保留为 FP32 保护路径。保护层采用 FP32 的依据既包括前述敏感性分析结果，也与当前 MNN CPU 混合精度执行路径对量化豁免层的实现方式保持一致；也就是说，这里并非额外设计独立的 FP16 激活量化路径，而是让方法选择与工具链实现保持一致。

在具体实现上，主体卷积采用 INT8 权重与激活联合量化，权重量化选用 per-channel 对称方案，以适配各输出通道在 BN 融合后仍存在的幅值差异，并避免 ARMv8.2 dotprod 的零点偏置修正开销。EFEM 的投影与融合卷积参与 INT8 校准，但 gather/correlation 本身不作为独立 INT8 加速对象处理；后训练量化的直方图校准流程放在 4.3.2 中单独说明，具体分层方案如表~\ref{tab:precision_allocation} 所示。
...
本文采用后训练量化（PTQ）生成可部署的混合精度模型，而不引入量化感知训练（QAT）。原因在于主体卷积的统计分布较稳定，适合通过离线校准确定量化区间；高风险激活与几何求解链路则已保留为 FP32。图~\ref{fig:ptq_pipeline} 展示了从 PyTorch 模型出发，经 ONNX 导出、\texttt{mnnconvert} 图优化、代表集校准到真机验证的部署流程，其中 DWT 求解器以旁路 FP32 形式运行，不进入 ONNX/MNN 量化图。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{images/PTQPipeline}
  \caption{混合精度量化 PTQ 部署流程图}
  \label{fig:ptq_pipeline}
\end{figure}

表~\ref{tab:ptq_config} 汇总本文 PTQ 的关键配置。本文借鉴 NCNN 的直方图校准策略：以代表性样本逐层统计激活分布，搜索各层 INT8 截断阈值，并将得到的 scale/clip 范围写入量化配置；EFEM gather/correlation、EFEM scores、热力图/置信度输出与 DWT 通过豁免列表保留 FP32。模型转换完成后，仅进行一致性检查与真机验证，不再引入额外训练。

本文所采用的 PTQ 校准流程借鉴 \texttt{ncnn2table} 思路，其核心是利用激活直方图而非直接采用极值来确定截断阈值。本文从 RT-Pose 训练集中划分 256 组不参与训练的双目样本作为代表性校准集，并保持与部署阶段一致的输入分辨率 $640\times480$ 与 batch=1 预处理流程。校准时，逐层统计骨干卷积与 EFEM 投影/聚合层的激活直方图，枚举候选截断阈值，并以 Kullback-Leibler（KL）散度衡量量化分布与原始分布之间的信息损失；使 KL 散度最小的阈值被选作最终 clip 值，再换算为激活 scale。相较于简单 min-max，该方法对异常大值更稳健，更适合本文中存在长尾响应的跨视图特征张量。算法流程如算法~\ref{alg:ncnn_hist_ptq} 所示。
...
需要指出的是，该直方图校准方法仅用于低敏感层的 INT8 区间估计，而非对全网络统一量化。根据 4.2.3 节的敏感性结论，EFEM gather/correlation、EFEM softmax 前注意力分数、热力图/置信度输出以及 DWT 求解器在校准前即被移出统计列表，不参与直方图建表。因此，本文的 PTQ 流程可概括为两步：先按敏感性确定可量化层集合，再对该集合执行直方图校准。
...
对比配置。 本节测试四种精度配置：（1）FP32 全精度，作为精度上界与速度下界；（2）FP16 全精度（第三章基线）；（3）激进 INT8，在主体卷积量化基础上进一步扩大激活量化覆盖范围以追求最大加速，DWT 保留 FP32；（4）本文混合精度，依据表~\ref{tab:precision_allocation} 仅对低敏感主体卷积执行 INT8 权重/激活量化，对高敏感激活与求解器保持 FP32。四种配置共享相同的网络结构、输入尺寸与推理协议，仅精度设置不同；因此其性能差异可主要归因于量化策略本身。
...
表~\ref{tab:quant_comparison} 反映出三点。其一，FP32 与 FP16 的 MPJPE 完全一致，而 FP16 延迟仅为 FP32 的约 56\%，说明本任务在端侧采用 FP16 不会带来可观测精度损失。其二，激进 INT8 虽取得最高加速比（1.71$\times$），但 MPJPE 增量达到 $+13.4\,\text{mm}$，说明对高敏感激活继续量化会显著破坏跨视图匹配与坐标回归精度。其三，本文混合精度以 72.3\,ms 的延迟实现 1.60$\times$ 加速，同时仅带来 $+2.2\,\text{mm}$ 的 MPJPE 增量，表明主要收益已由主体卷积层的低风险量化获得，而关键保护层有效抑制了额外精度损失。综合而言，本文方案保留了激进 INT8 的主要加速收益，同时显著抑制了精度退化，因此在目标平台上表现出更优的精度--效率折中。需要说明的是，这里的对比对象限定为同一网络在不同精度配置下的部署表现，其目标是验证本文平台内量化决策框架的有效性，而非宣称对所有移动端姿态估计量化方案的绝对优越性。
