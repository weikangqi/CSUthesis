...
因此，本文关注的问题并不是单纯追求三维姿态估计的精度上限，而是如何围绕双目几何信息设计更轻量、更高效且更适合端侧落地的三维人体姿态估计框架。一方面，需要避免将整图稠密深度估计的计算负担机械引入姿态任务；另一方面，还需要兼顾模型压缩、推理效率与部署友好性，使方法能够在资源受限设备上保持可接受的精度--时延平衡。深度网络模型压缩研究表明，结构设计与压缩策略需要协同考虑，才能在精度保持与实际加速之间取得更稳健的平衡\citess{gao2021deepcompresscn}。同时，面向边缘智能的协同推理研究也说明，部署阶段的算子组织方式与运行环境差异会进一步影响最终落地效果\citess{wang2023edgecollabcn}。\textcolor{blue}{开展面向资源受限设备的轻量化双目三维人体姿态估计研究，既具有明确的学术问题导向，也具有重要的工程应用价值。}

\subsection{国内外研究现状}
\textcolor{blue}{本节围绕本文研究主线，对三维人体姿态估计、双目深度恢复以及端侧轻量化部署三个方面的相关工作进行梳理，并在总结代表性方法的基础上归纳现有研究仍然存在的问题。}
\subsubsection{\textcolor{blue}{单目三维人体姿态估计研究现状}}
\textcolor{blue}{从传感器配置看，三维人体姿态估计可大致分为单目、双目与多目三类路线。}单目方法硬件成本低、部署灵活，因此长期是三维姿态估计研究的主流方向。
...
\subsubsection{\textcolor{blue}{多目三维人体姿态估计研究现状}}
...
对于本文任务而言，上述立体匹配研究的重要启发在于：双目深度恢复的主流高精度方案大多依赖代价体、全视差搜索和多轮特征聚合，而这些模块恰恰也是端侧部署中最主要的算力与访存开销来源。因此，当双目视觉被用于人体三维姿态恢复时，若仍直接沿用稠密视差估计范式，往往会带来较重的系统负担。\textcolor{blue}{这也说明，双目三维姿态估计不能简单等同于“人体关键点检测 + 立体匹配”的串联，而需要围绕关节任务重新审视双目交互方式和几何恢复路径。}
\subsubsection{\textcolor{blue}{双目三维人体姿态估计研究现状}}
\textcolor{blue}{双目三维人体姿态估计通常沿“二维关键点检测 + 深度/视差先验 + 三维回代”的路线展开，即先通过双目匹配恢复深度信息，再结合人体关键点位置完成三维关节求解。}
...
Zhang 和 Guo 提出 DMCPose\citess{zhang2024dmcpose}，以前端高精度二维姿态网络提取双目关键点观测，再结合关键点级视差、多视图一致性与注意力机制进行动态融合，从而绕开传统稠密立体匹配的整图搜索过程。\textcolor{blue}{该方法说明双目三维人体姿态估计可以围绕人体关节建立更轻量的几何恢复路径，}是近年来“双目几何先验 + 注意力融合”思路的代表性工作之一；但其验证场景仍以受控数据集上的单人动作为主，尚未进一步覆盖多人实时场景与资源受限端侧部署条件。
...
\textcolor{blue}{总体来看，现有双目三维人体姿态估计方法仍以级联方案为主，二维检测误差与深度估计误差容易耦合，整图深度估计开销偏重，这些因素共同限制了其在端侧场景中的进一步优化空间，也正是本文所要重点解决的核心问题。}
...
Human3.6M\citess{ionescu2014human36m} 是三维单人人体姿态估计领域目前最常用的大型室内基准数据集。数据集由 11 名专业演员（6 男 5 女）在受控室内环境下执行讨论、拍照、打电话等 17 种日常活动，通过 4 台同步 RGB 摄像机采集，包含约 360 万帧图像及对应的二维、三维关节标注，同时提供精确的相机内外参数。数据集以 MPJPE（Mean Per Joint Position Error）为主要评价指标，是单目、双目及多视角三维人体姿态估计方法进行横向精度对比的核心标准基准。

CMU Panoptic\citess{joo2015panoptic} 是多视角多人三维人体姿态估计领域最常用的大型基准数据集，由 Joo 等人于 2015 年提出。数据集利用 480 台同步摄像机（包含 RGB 相机和 Kinect 深度相机）在受控穹顶环境中采集了多人社交场景，提供 31 个 RGB 视角的时序数据及三维骨骼标注，覆盖多人互动、交谈等复杂社交行为。该数据集是 VoxelPose、TEMPO、MVGFormer 及 MV-SSM 等多目三维姿态估计方法的主要评测基准。

MADS\citess{zhang2017mads}（Martial Arts, Dancing and Sports Dataset）是专为双目与多视角三维人体姿态估计设计的挑战性数据集，由 Zhang 等人发布。数据集采用一个双目相机及 2 个单目相机（共 4 视角）记录了单人在武术、舞蹈和体育运动场景中的大幅度动作，包含 53\,000 帧 RGB 图像及对应的三维关节真值，提供完整的双目相机内外参数。MADS 是目前少数面向双目相机配置的三维人体姿态估计基准之一，对双目方法的特定评测具有重要参考价值。

3DPW\citess{vonmarcard2018recovering}（3D Poses in the Wild）是首批面向户外复杂场景的三维人体姿态数据集，采用手持单目相机与可穿戴 IMU（惯性测量单元）组合采集，覆盖城市街道、草坪等多种户外环境，包含 5.1 万帧 RGB 图像及对应的 SMPL 三维网格与关节标注。3DPW 主要用于评估模型在自然场景下的跨域泛化能力，常作为单目三维人体恢复方法的域外测试基准。

RT-Pose\citess{ho2024rt} 是 Ho 等人于 2024 年发布的多模态三维人体姿态估计与定位基准数据集（ECCV 2024）。数据集同步采集双路 RGB 相机、非重复水平扫描 LiDAR 及级联成像 4D 雷达三类传感器数据，涵盖 10 名参与者在 40 个场景下（包括室内整洁/杂乱环境以及室外正常天气、雨天和弱光等条件）完成的 6 种不同复杂度动作，共计 7.2 万帧、240 个序列、总时长约 120 分钟，提供与 Human3.6M 兼容的三维骨架标注。三维真值通过将 HRNet 预测的二维关键点输入 ZeDO 优化框架、以 LiDAR 点云辅助根节点深度定位并经人工校验生成，具有较高的标注精度。该数据集的双路 RGB 相机提供了完整的内外参标定信息，支持将其作为双目相机配置用于三维姿态估计，是目前少数同时具备真实室内外场景多样性与精确双目标定的三维人体姿态基准之一。本文在实验中仅使用其双目 RGB 视频流（不使用雷达与 LiDAR 模态），将其作为轻量化双目三维人体姿态估计方法的核心评测基准。
...
\textcolor{blue}{针对现有双目三维人体姿态估计方法在端侧部署中面临的稠密计算冗余与部署开销偏大两大核心问题，本文采用软硬件协同设计的研究思路，在网络架构设计阶段即将目标硬件的算子约束、内存访问模式与部署友好性纳入考量，使轻量化算法设计与硬件感知部署形成闭环优化，提出了一套从特征提取、跨视图交互、三维求解到混合精度量化压缩的完整端侧解决方案，具体包含以下两个方面。}

\paragraph{轻量化非对称双目三维人体姿态估计网络设计}
鉴于现有双目方案大多沿用"稠密视差估计＋关节回代"的级联思路，存在大量与关节定位无关的像素级计算，且左右对称骨干结构在辅助视角存在明显容量冗余，本文提出了一种轻量化非对称双目学习框架。在骨干设计上，采用非对称骨干配置，以大容量主分支负责主视角语义表征，以轻量辅助分支提供稳定的几何观测，并引入跨分支一致性约束保持辅助分支的表征质量。在跨视图交互上，提出\textcolor{blue}{极线特征增强模块（Epipolar Feature Enhancement Module, EFEM）}，将跨视图匹配限制在极线约束下的一维视差搜索空间，避免构建全分辨率代价体。在三维恢复上，设计\textcolor{blue}{可微加权三角化姿态求解器（Differentiable Weighted Triangulation, DWT）}，通过 soft-argmax 提供可微二维观测，并由置信度权重对遮挡或误检视角降权，将热力图分支与几何求解统一到端到端训练链路中。上述架构决策在降低理论计算量的同时，已兼顾目标平台 INT8 量化路径的算子适配性，为后续硬件感知部署提供结构基础。

\paragraph{面向端侧平台的硬件感知量化压缩}
理论参数量与 FLOPs 的降低并不必然转化为真实端侧时延收益，仍需针对目标硬件的算子支持、内存访问模式与数值精度约束进行定向优化。本文面向 Snapdragon~8~Gen3 CPU 平台，首先通过模块级延迟实测与 Roofline 分析定位推理瓶颈（骨干主要受算力约束，EFEM gather 更接近访存约束），继而通过逐层量化敏感性分析识别精度敏感层（EFEM 注意力激活、热力图头、DWT 求解层），据此制定骨干卷积 INT8、敏感层保留 FP32 的分层混合精度方案，并经后训练量化（PTQ）流程输出可在 MNN 框架上运行的部署模型，在真机上验证精度保持与推理加速的有效平衡；\textcolor{blue}{同时，本文进一步给出面向 NVIDIA Jetson AGX Xavier 开发套件的平台验证与系统展示，为后续嵌入式 GPU 平台实测验证预留接口。}
\subsection{论文组织结构}
第一章为绪论。介绍本文的研究背景与现实意义，分析\textcolor{blue}{双目三维人体姿态估计}在端侧场景中面临的主要挑战，综述国内外相关研究现状，最后阐述本文的研究内容、主要贡献与论文组织结构。
...
第三章为\textcolor{blue}{基于双目相机的轻量化双目三维人体姿态估计网络设计。提出一种轻量化非对称双目学习框架：以非对称骨干网络消除双分支对称冗余，以极线特征增强模块（EFEM）将跨视图匹配限制在一维极线搜索空间，以可微加权三角化姿态求解器（DWT）实现端到端的三维关节坐标恢复，并通过对比实验与消融实验验证各模块的有效性，}如图~\ref{fig:system_overview}所示。

第四章为硬件感知部署与量化压缩。以 Snapdragon 8 Gen3 CPU 为已完成定量验证的平台，通过延迟拆解与 Roofline 模型定位推理瓶颈，借助量化敏感性分析制定分层混合精度量化方案，经后训练量化（PTQ）流程输出可部署的 MNN 模型，并在真机上验证精度保持与推理加速效果；\textcolor{blue}{此外，本章还给出面向 NVIDIA Jetson AGX Xavier 开发套件的平台验证与系统展示，为后续补充嵌入式 GPU 数据做好准备。}
...
本章围绕双目轻量化三维人体姿态估计的研究主线，系统梳理相关核心理论与关键技术，为方法设计与实验分析提供知识铺垫。全章内容分为四个递进部分：第一，介绍卷积神经网络与Transformer注意力机制等深度学习基础，阐述其在视觉特征提取与跨视图信息融合中的作用原理；第二，梳理人体姿态估计的任务范式、关键点表示方法与评价指标体系，\textcolor{blue}{明确双目三维姿态估计的输出定义与评测标准；}第三，阐述双目立体视觉的成像模型与极线几何约束，介绍三角测量深度恢复的基本原理；第四，总结面向端侧部署的轻量化结构设计原则与模型压缩策略，概述在资源受限设备上实现高效推理的主要技术路径。
...
卷积神经网络（Convolutional Neural Network, CNN）是计算机视觉领域最常用的基础模型之一，尤其擅长处理具有网格状拓扑结构的数据，如图像（2D网格）和视频（3D网格）。CNN的核心思想在于通过模仿生物视觉系统的机制，有效利用图像的空间局部相关性，并通过一系列专门的层级结构来学习和提取从低级到高级的层次化特征。
\begin{figure}[htbp]
  \centering
...
\paragraph{卷积层}
卷积层是CNN(Convolutional Layer)的核心。它使用一组可学习的滤波器（或称为卷积核, Kernel）在输入数据（如图像）上进行滑动窗口式的卷积操作。每个卷积核负责检测一种特定的局部特征，例如边缘、角点、纹理或颜色块。通过卷积操作，可以生成一个二维的特征图，图中的每个像素代表了其在原始图像对应感受野（Receptive Field）上对该特定特征的响应强度。卷积层有两个非常重要的特性：
...
Transformer架构最初由Vaswani等人在其著名的论文“Attention Is All You Need”\citess{vaswani2017attention}中提出，旨在解决自然语言处理（NLP）中的序列到序列（Seq2Seq）任务。它摒弃了传统的循环神经网络（RNN）和卷积神经网络（CNN）的序列处理方式，完全依赖于注意力机制来捕捉输入和输出序列之间的全局依赖关系。由于其强大的长距离依赖建模能力和并行计算潜力，Transformer及其变体（如Vision Transformer, ViT\citess{dosovitskiy2020image}）已被成功应用于计算机视觉领域，并在包括人体姿态估计在内的多项任务中取得了SOTA（State-of-the-art）性能。
...
面向移动端或嵌入式设备的视觉算法受到算力、内存带宽与功耗的严格约束。对于\textcolor{blue}{双目三维人体姿态估计}而言，网络需在参数量、计算量（FLOPs）、峰值内存与精度-效率折中四个维度协同权衡，同时避免传统立体匹配中高维代价体（Cost Volume）带来的显存开销。国内关于轻量级神经网络架构的高水平综述指出，端侧模型设计不能只关注参数量缩减，还需要兼顾算子友好性与实际部署效率\citess{zhu2020lightweightnncn}。关于深度神经网络压缩与加速的系统研究进一步表明，端侧优化还应同时考虑访存压力与硬件执行特性\citess{ji2018compressioncn}。深度网络模型压缩综述则从压缩策略适用边界角度强调，结构设计与压缩策略需要协同考虑\citess{gao2021deepcompresscn}。轻量化设计通常遵循"减少高维张量、优先2D算子、控制通道数与分辨率"的总体原则。本小节从结构轻量化设计、模型压缩与端侧部署工程优化三个层次依次介绍在边缘端实现高效推理的常用方法。
...
结构轻量化与模型压缩确定了网络的理论效率上限，而端侧实际推理性能还取决于算子与硬件的匹配程度。端侧推理的瓶颈往往不仅来自算术计算量，还来自内存带宽与访存模式：频繁的特征拼接与大体积张量读写会显著抬高时延与能耗，双目任务中代价体易导致内存峰值过高，需优先以2D算子替代高维操作。工程层面应结合算子融合（Operator Fusion）、张量布局优化与内存复用等手段，尽量避免大规模3D卷积、超大矩阵乘等端侧不友好算子，才能将理论压缩收益转化为稳定的端侧加速。综合来看，轻量化网络设计需要在结构设计、压缩策略与部署工程三个层面协同优化，才能在资源受限设备上实现可用的\textcolor{blue}{双目三维人体姿态估计}性能。


\subsection{本章小结}

本章系统梳理了本文研究所依赖的核心理论与技术基础。在深度学习方面，介绍了卷积神经网络的层次特征提取机制，以及 Transformer 的多头注意力与交叉注意力框架，它们分别适用于高效视觉特征提取和跨位置、跨视图的全局关联建模。在人体姿态估计方面，归纳了2D/3D姿态估计的主流范式、热力图与坐标回归两类表示方法，以及二维与三维姿态估计的常用评价指标，明确了\textcolor{blue}{双目三维姿态估计}的任务定义与评测体系。在双目立体视觉方面，阐述了针孔成像模型、极线几何约束与三角测量深度恢复方法，为高效跨视图特征匹配与三维坐标恢复奠定了几何基础。在轻量化网络设计方面，总结了深度可分离卷积、倒残差结构等高效骨干范式，以及量化、剪枝与知识蒸馏等模型压缩策略，为面向资源受限设备的视觉算法设计提供了理论参照。

\clearpage
\section{\textcolor{blue}{基于双目相机的轻量化三维人体姿态估计网络设计}}
...
\textcolor{blue}{然而，双目几何能够缓解深度歧义，并不意味着模型在端侧部署时就天然具备低开销与高实时性。}现有许多双目深度或三维重建方法仍依赖高维代价体构建与密集正则化，往往需要三维卷积或大规模迭代优化才能获得较高精度，这会带来显存占用高、访存压力大与推理延迟不可控等问题\citess{chang2018pyramid}。面向移动平台的立体匹配研究则进一步表明，为获得更可控的部署成本，必须主动压缩跨视图交互与代价聚合链路\citess{shamsafar2022mobilestereonet}。另一方面，若直接采用左右对称的双分支结构分别完成特征提取与姿态预测，骨干网络与多尺度融合模块的计算量将近似倍增，导致大量跨视图冗余计算；在多人场景中，该冗余会随实例数增加而放大，进一步削弱端侧实时性，并使系统时延随场景复杂度产生明显波动。
...
\textcolor{blue}{综上，本章的核心目标是在保留双目几何优势的同时，围绕端侧约束重构高效的关节级推理链路，并通过后续的问题分析、网络设计与实验验证给出完整解决方案。}
\subsection{问题分析}
...
\textcolor{blue}{针对 3.2 节归纳的两类瓶颈，即稠密级联开销（问题A）与左右对称冗余（问题B），本节提出轻量化非对称双目学习框架。}对于双目三维人体姿态估计而言，任务本质上是围绕同一组人体关节完成二维定位、跨视图对应与三维恢复，而不是让左右视图分别承担一套完全对等的重型三维理解流程。基于这一点，本文采用“主分支负责稳定语义与几何参照、辅助分支负责提供跨视图匹配线索”的非对称分工，将有限计算资源集中到对三维恢复更关键的表征上。

在此基础上，本文从骨干架构、跨视图交互、三维求解与训练约束四个层面构建完整方法：以非对称双目骨干网络（第~\ref{subsubsec:asym_backbone}节）压缩左右分支冗余，以极线特征增强模块（第~\ref{subsubsec:efem}节）建立受几何约束的低开销跨视图匹配，以可微加权三角化姿态求解器（第~\ref{subsubsec:dwt}节）实现端到端三维恢复，并通过跨分支表征一致性约束（第~\ref{subsubsec:align_loss}节）弥补轻量分支的表示能力损失。整体设计遵循“先压缩冗余表征、再增强跨视图几何、继而完成可微三维恢复、最后补偿轻量分支能力”的展开逻辑。

\subsubsection{整体框架与端到端流程}
\label{subsubsec:overall_pipeline}
\textcolor{blue}{针对 3.2.3 节总结的稠密级联开销与左右对称冗余问题，本文提出轻量化双目三维人体姿态估计框架，其整体流程如图~\ref{fig:pipeline_ch3} 所示。输入为立体校正后的左右视图，首先经非对称双目骨干提取多尺度特征；随后在主尺度上引入极线特征增强模块，在一维视差搜索空间内完成跨视图交互，得到增强表征；最后利用可微加权三角化姿态求解器将左右二维观测与相机模型融合为三维关节坐标，并在训练阶段配合跨分支表征一致性约束提升轻量分支的表征能力。整体上，所提框架以“非对称特征提取 + 极线约束交互 + 可微几何求解”为主线，在避免稠密代价体构建的同时实现端到端三维恢复。}
...
\midrule
\multicolumn{9}{c}{\textcolor{blue}{双目基线方法（双目$(I_l,I_r)$）}} \\
\midrule
MobiDepthPose~\citess{zhang2022mobidepth} & 221.3 & 215.7 & 207.5 & 208.9 & 223.3 & 215.3 & 7.27 & 10.36 \\
\textcolor{blue}{DMCPose~\citess{zhang2024dmcpose}} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} \\
...
\textcolor{blue}{表~\ref{tab:rtpose_benchmark} 给出了 RT-Pose 上的对比结果，并将 DMCPose 纳入双目基线集合。当前 DMCPose 在本文统一训练与评测协议下的复现实验仍在补充，因此表中暂以占位形式保留其基线位置。}总体上，本文方法在平均指标上达到 $158.6$\,mm，显著优于单目基线与已完成复现的双目基线：相较于最佳单目方法 RTMW3D（$307.3$\,mm），绝对误差降低 $148.7$\,mm；相较于当前已完成结果中最优的双目级联基线 RTMO+MobileStereoNet（$202.0$\,mm），绝对误差降低 $43.4$\,mm。进一步地，在坐姿等遮挡更强、远距离小视差更敏感的场景下，级联范式易受稠密深度误匹配与关节采样不稳定影响（例如 $200.5$\,mm 降至 $132.6$\,mm，降低 $67.9$\,mm），而本文通过端到端的双目特征交互与可微几何求解将计算集中于关节相关区域，从而获得更稳定的三维定位精度。
...
\midrule
Simple Baseline~\citess{martinez2017simple} & {\color{blue}--} \\
JointFormer~\citess{lutz2022jointformer} & {\color{blue}--} \\
RTMW3D~\citess{jiang2024rtmw} & {\color{blue}--} \\
MobiDepthPose~\citess{zhang2022mobidepth} & {\color{blue}--} \\
\textcolor{blue}{DMCPose~\citess{zhang2024dmcpose}} & {\color{blue}--} \\
RTMO~\citess{lu2024rtmo}+SGBM$^\dagger$~\citess{hirschmuller2005accurate} & {\color{blue}--} \\
RTMO~\citess{lu2024rtmo}+MobileStereoNet~\citess{shamsafar2022mobilestereonet} & {\color{blue}--} \\
{\color{blue}本文方法} & {\color{blue}33.68} \\
\bottomrule
\end{tabular}
\end{table}

\textcolor{blue}{为与 RT-Pose 主实验保持一致，表~\ref{tab:mads_generalization} 列出了相同的对比方法，并补充保留了 DMCPose 的基线位置。当前仅完成本文方法的零样本测试，其余方法结果暂不填，后续将在统一协议下补充。} \textcolor{blue}{在不经任何目标域微调的情况下，本文方法在 MADS 数据集上取得 33.68 mm 的平均 MPJPE，说明所提轻量化双目框架对不同动作分布与采集协议变化具有一定鲁棒性。}
...
图~\ref{fig:latency_vs_people} 与表~\ref{tab:latency_vs_people} 表明，本文方法在 1 人到 10 人场景下的端到端推理时延仅由 116.0\,ms 增至 117.3\,ms，近似保持人数无关；相比之下，RTMW3D 由于包含逐实例裁剪与回归，时延随人数显著上升，而 RTMO+MobileStereoNet 虽也保持近似恒定，但整体仍稳定慢于本文方法。该结果说明本文的主要计算仍集中在整图前向与稀疏关节级三维求解上，因此能够在保持较好精度的同时获得更稳定的多人场景可扩展性。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{images/latency_vs_people.pdf}
\caption{各方法推理时延随人数变化}
...
\subsubsection{方法局限性与适用边界}
尽管第三章结果验证了所提框架的有效性，但其结论仍具有明确的适用边界，需要在论文中加以说明。

第一，本文方法对极端遮挡、远距离小视差和弱纹理场景仍然敏感。由第二章式\eqref{eq:depth_uncertainty}可知，当目标距离增加、有效视差减小时，固定像素级扰动会被放大为更显著的深度误差；此时即使 EFEM 与 DWT 仍可提供稳定收益，三维恢复上限也会同步下降。

第二，本文框架依赖双目标定与立体校正质量。EFEM 的一维极线搜索和 DWT 的显式投影求解都默认相机内外参与同步误差处于可控范围内；若外参漂移、残余垂直视差或时间不同步显著增大，模型输出会首先在跨视图匹配与三角化环节受到影响。

第三，当前泛化验证仍以 RT-Pose 训练、MADS 零样本测试为主。需要指出的是，这一边界更多来自公开双目三维人体姿态基准本身的稀缺性，而非实验设计上忽略了泛化验证：现阶段同时满足“精确双目标定、真实多人场景、统一骨架标注、允许公开复现实验”这几项条件的数据集数量非常有限。因而，本文已在当前可获得条件下尽量覆盖“真实场景主基准 + 跨数据集零样本测试”两类验证，但论文中的泛化结论仍应理解为“在现有公开数据条件下的初步验证”，而非对所有双目场景的普遍保证。

第四，本文当前结论主要针对静态图像级双目输入和单帧几何恢复链路成立，尚未引入视频时序建模、跨帧跟踪与长时稳定性约束。因此，关于多人时延稳定性与端侧部署有效性的结论应限定为“整图单帧推理链路上的稳定性”，而不宜直接外推为长视频场景下的全流程最优解。

\subsection{本章小结}

本章围绕端侧多人三维人体姿态估计任务，针对现有双目方法在稠密级联开销与左右对称冗余两方面的瓶颈，提出了一种轻量化非对称双目学习框架。

方法上，本文并未沿用“稠密深度估计+关节回代”的重型级联思路，而是围绕关节级几何任务重新组织双目信息流：一方面通过非对称骨干配置与跨分支一致性约束压缩冗余计算并保持辅助分支表征能力；另一方面通过 EFEM 建立受极线约束的低开销跨视图匹配，并结合可微加权三角化姿态求解器（DWT），将二维热力图观测、视图置信度与显式投影约束统一到端到端三维恢复链路之中。
...
本文第三章的主要实验仍以 RT-Pose 为核心验证基准，并进一步补充了 RT-Pose$\rightarrow$MADS 的零样本泛化测试。需要强调的是，当前尚未覆盖更多双目采集协议，并不意味着本文缺少对泛化问题的关注，而是因为具备公开标定参数、统一三维骨架标注且可直接支撑端侧双目姿态评测的数据集本身较为缺失；在这一现实约束下，本文已优先选择最具代表性的真实场景数据进行验证。对于标定扰动、外参误差与更广泛采集条件差异的系统量化，仍有待后续在更丰富基准上进一步展开。

第三章的关键贡献在于证明：在不依赖稠密深度中间表示的前提下，双目三维人体姿态估计仍可通过结构轻量化、几何约束驱动的跨视图交互与可微三维求解实现精度与效率的统一。\textcolor{blue}{更细粒度的部署资源指标，如平台侧内存占用与功耗/能耗特征，将在第四章 Jetson AGX Xavier 平台验证中进一步补充。}这也为下一章进一步开展硬件感知部署与量化压缩提供了方法基础。
\newpage
\section{硬件感知部署与量化压缩}
...
从任务建模角度看，双目方案之所以值得关注，在于其在几何约束强度与工程落地成本之间提供了更均衡的折中。单目三维姿态估计虽然硬件成本最低，但在复杂遮挡、尺度变化与大幅动作条件下容易受到深度歧义与绝对尺度不确定性的限制\citess{yu2024posecodeccn}。多目系统虽然能够提供更强的空间约束，却往往伴随更高的硬件成本、更复杂的同步标定流程以及更苛刻的安装条件，而代表性多视角方法也体现出跨视图融合与系统搭建开销较大\citess{tu2020voxelpose}。双目系统能够借助视差与三角测量显式引入深度线索，在硬件复杂度、部署成本与三维恢复稳定性之间取得更可行的工程平衡\citess{hirschmuller2005accurate}。
...
除高性能手机 CPU 外，\textcolor{blue}{嵌入式开发板上的 GPU 平台}也是边缘落地的重要载体。这类平台通常具有较强的并行计算能力和更灵活的本地接入方式，适合部署在机器人终端、固定式感知节点与轻量边缘设备之中。然而，\textcolor{blue}{GPU 友好}并不等价于“模型可直接迁移”：算子支持范围、图优化方式、低比特量化兼容性以及 GPU/CPU 异构回退都会影响最终部署效果。基于这一考虑，本文在完成 Snapdragon 8 Gen3 CPU 平台上的量化验证之外，\textcolor{blue}{进一步给出面向 NVIDIA Jetson AGX Xavier 开发套件的平台验证与系统原型设计，以便后续围绕嵌入式 GPU 的实测指标开展补充评估。}
...
\textcolor{blue}{本文从 RT-Pose 训练集中划分 256 组双目样本作为代表性校准集，输入分辨率固定为 $640\times480$，batch=1。}校准时，逐层统计骨干卷积与 EFEM 投影/聚合层的激活直方图，枚举候选截断阈值，并以 Kullback-Leibler（KL）散度衡量量化分布与原始分布之间的信息损失；使 KL 散度最小的阈值被选作最终 clip 值，再换算为激活 scale。相较于简单 min-max，该方法对异常大值更稳健，更适合本文中存在长尾响应的跨视图特征张量。算法流程如算法~\ref{alg:ncnn_hist_ptq} 所示。
...
\textcolor{blue}{除上述 CPU + MNN 部署链路外，本文还同步设计了面向 NVIDIA Jetson AGX Xavier 开发套件的扩展路径，即 `PyTorch $\rightarrow$ ONNX $\rightarrow$ TensorRT engine 构建 $\rightarrow$ TensorRT Runtime 装载 $\rightarrow$ Jetson AGX Xavier 平台侧部署`。TensorRT 支持在单一 engine 内按层组织混合精度推理，因此主体卷积层可优先映射至 FP16 或 INT8 路径执行，而对数值更敏感的 softmax、热力图输出和几何求解相关模块则可保留为更高精度，必要时再通过 GPU/CPU 异构回退接口处理暂不支持算子。需要说明的是，Jetson AGX Xavier 部署部分当前重点在于完成链路设计与系统接入，相关性能与精度评估结果将在后续实验中补充。}

\subsection{实验评估}

\textcolor{blue}{本节首先给出已完成的 CPU 平台定量实验结果，包括不同精度配置下的精度--效率权衡、精度敏感层消融以及多人场景下的时延稳定性分析；随后补充面向 NVIDIA Jetson AGX Xavier 开发套件的部署验证、系统展示以及资源开销评估。由于 Jetson AGX Xavier 平台的精度与资源实测数据仍在补充中，相关内容当前以部署链路打通、测试协议说明和正式表位预留为主，不对平台性能作定量结论。}

\subsubsection{实验设置}

硬件平台与推理环境。已完成的定量实验均在 Xiaomi\,14 真机（Snapdragon 8 Gen3，16\,GB LPDDR5X）上进行，推理框架采用 MNN 2.9.0，启用 ARMv8.2 dotprod 与 FP16 NEON 支持。所有实验均固定 4 线程 CPU 执行环境，预热 10 次后正式计时 100 次，取均值作为延迟报告值；批大小固定为 1，输入分辨率统一为 $640\times480$。\textcolor{blue}{面向 NVIDIA Jetson AGX Xavier 开发套件的平台验证不纳入本节定量对比，其部署环境与系统展示将在 4.4.5 节和 4.4.6 节给出。}

对比配置。本节测试四种精度配置：FP32 全精度、FP16 全精度（第三章基线）、激进 INT8，以及本文混合精度。四种配置共享相同的网络结构、输入尺寸与推理协议，仅精度设置不同。

评估指标。精度以 RT-Pose 验证集上的 MPJPE 均值（mm）衡量，同时记录与 FP16 基线的偏差 $\Delta\text{MPJPE}$；效率以端到端推理延迟（ms，单人场景）和模型文件大小（MB）表征。\textcolor{blue}{对于 4.4.5 节和 4.4.6 节的 Jetson AGX Xavier 平台验证，除沿用相同的精度指标外，还将补充平台侧内存占用与功耗/能耗指标。其中，内存占用以推理进程峰值 RSS 与稳态系统内存占用为主，功耗以固定功耗模式下采样窗口内的平均板载功率为主，并进一步换算单帧能耗；这些指标仅用于 Jetson 平台内部的部署代价评估，不与 Xiaomi 14 真机结果直接作横向比较。}
...
\subsubsection{\textcolor{blue}{Jetson AGX Xavier平台验证}}
\textcolor{blue}{为验证所提方法在嵌入式 GPU 平台上的可部署性，本文选取 NVIDIA Jetson AGX Xavier 开发套件作为验证对象，重点完成模型转换、运行时接入与基本推理链路打通；其验证环境与软件配置如表~\ref{tab:jetson_env} 所示。}

\begin{table}[htbp]
  \centering
  \small
\caption{\textcolor{blue}{Jetson AGX Xavier验证环境与软件配置}}
  \label{tab:jetson_env}
  \begin{tabular}{@{}p{0.18\textwidth}p{0.26\textwidth}p{0.42\textwidth}@{}}
    \toprule
    项目 & 配置 & 说明 \\
    \midrule
    \textcolor{blue}{验证平台} & \textcolor{blue}{NVIDIA Jetson AGX Xavier 开发套件} & \textcolor{blue}{作为嵌入式 GPU 验证对象} \\
    模型输入 & $640\times480$，batch=1 & 与 CPU 平台保持一致 \\
    \textcolor{blue}{模型来源} & \textcolor{blue}{ONNX 导出模型} & \textcolor{blue}{由 PyTorch 模型导出后供 TensorRT 工具链转换} \\
    \textcolor{blue}{转换工具} & \textcolor{blue}{TensorRT} & \textcolor{blue}{负责 engine 构建与图优化} \\
    \textcolor{blue}{运行时} & \textcolor{blue}{TensorRT Runtime} & \textcolor{blue}{负责平台侧装载与执行} \\
    \textcolor{blue}{精度配置} & \textcolor{blue}{FP16 / INT8 / 混合精度} & \textcolor{blue}{支持在单一 TensorRT engine 内按层配置} \\
    \textcolor{blue}{运行方式} & \textcolor{blue}{GPU 主路径 + 必要时 CPU 回退} & \textcolor{blue}{为不支持算子预留异构执行接口} \\
    \bottomrule
  \end{tabular}
\end{table}

\textcolor{blue}{验证采用 `PyTorch $\rightarrow$ ONNX $\rightarrow$ TensorRT engine $\rightarrow$ TensorRT Runtime $\rightarrow$ Jetson AGX Xavier` 的部署链路，在平台侧完成双目图像输入、模型调用与结果回传。部署时可在同一 TensorRT engine 内联合使用 FP16、INT8 与保留高精度的敏感层配置，以兼顾吞吐效率与数值稳定性。当前该部分仅用于说明部署可行性，不展开跨平台性能分析，相关时延与精度数据将在后续补充。}

\textcolor{blue}{资源监测协议方面，Jetson 平台统一固定输入分辨率为 $640\times480$、batch=1，并在相同 engine 配置下预热后再进入正式记录阶段。内存指标优先记录推理进程的峰值 RSS，并辅以系统稳态内存占用变化；功耗指标在固定电源模式与频率配置下，通过板载监测接口持续采样，取测试窗口内平均功率作为报告值，并进一步计算单帧能耗。所有资源指标均建议在同一精度配置下重复多次取均值；若波动较明显，则同时报告波动范围或标准差，以提高可复现性。}

\subsubsection{\textcolor{blue}{Jetson 平台资源开销评估}}
\textcolor{blue}{在 Jetson AGX Xavier 平台上，除推理精度与延迟外，内存占用和功耗/能耗同样是评估部署可用性的重要指标。与 CPU 平台侧的主实验不同，本节关注的是嵌入式 GPU 部署过程中的资源代价补充验证，即考察不同 TensorRT 精度配置在平台侧带来的内存驻留、平均功耗以及单帧能耗差异，而不将其外推为与手机平台之间的绝对优劣比较。}

\begin{table}[htbp]
  \centering
  \small
\caption{\textcolor{blue}{Jetson AGX Xavier平台资源开销评估}}
  \label{tab:jetson_resource_eval}
  \begin{tabular}{lcccc}
    \toprule
    \textcolor{blue}{精度配置} & \textcolor{blue}{延迟 (ms)$\downarrow$} & \textcolor{blue}{内存占用 (MB)$\downarrow$} & \textcolor{blue}{平均功耗 (W)$\downarrow$} & \textcolor{blue}{单帧能耗 (J/frame)$\downarrow$} \\
    \midrule
    \textcolor{blue}{FP16 engine} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} \\
    \textcolor{blue}{INT8 engine} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} \\
    \textcolor{blue}{混合精度 engine} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} & {\color{blue}--} \\
    \bottomrule
  \end{tabular}
\end{table}

\textcolor{blue}{表~\ref{tab:jetson_resource_eval} 为 Jetson 平台资源评估预留了正式结果位置。后续实验将至少覆盖 FP16 engine、INT8 engine 与混合精度 engine 三种配置，并在统一输入、统一电源模式和统一采样窗口下记录延迟、内存占用、平均功耗与单帧能耗。若某一精度配置因算子兼容性或数值稳定性限制暂未完成测试，则在表中保留占位并在正文中说明原因，而不直接补写推测性数值。}

\textcolor{blue}{就评估逻辑而言，Jetson 平台的资源测试主要用于回答两个问题：其一，混合精度 engine 在保持敏感层高精度的前提下，是否仍能显著降低平台侧内存驻留与平均功耗；其二，INT8 或混合精度配置带来的吞吐收益，是否能够进一步转化为单帧能耗下降。由于这些结果高度依赖 TensorRT 版本、功耗模式、频率锁定与采样工具设置，本文仅将其解释为特定 Jetson AGX Xavier 部署环境下的补充验证结论。}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \begin{tikzpicture}[x=1cm,y=0.8cm]
      ...
      \draw[fill=gray!20,draw=black] (0.6,0) rectangle (1.3,3.0);
      \draw[fill=gray!20,draw=black] (1.9,0) rectangle (2.6,3.0);
      \draw[fill=gray!20,draw=black] (3.2,0) rectangle (3.9,3.0);
      \node[font=\small] at (0.95,-0.45) {FP16};
      \node[font=\small] at (2.25,-0.45) {INT8};
      \node[font=\small] at (3.55,-0.45) {Mixed};
      \node[rotate=90,font=\small] at (-0.55,2.0) {Memory (MB)};
      \node[font=\small] at (2.25,3.45) {\textcolor{blue}{待补实测值}};
    \end{tikzpicture}
    \caption{\textcolor{blue}{内存占用对比占位图}}
    \label{fig:jetson_memory_placeholder}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\textwidth}
    \centering
    \begin{tikzpicture}[x=1cm,y=0.8cm]
      ...
      \node[font=\small] at (2.25,-0.45) {INT8};
      \node[font=\small] at (3.55,-0.45) {Mixed};
      \node[rotate=90,font=\small] at (-0.62,2.0) {Power / Energy};
      \node[font=\small] at (2.25,3.45) {\textcolor{blue}{待补实测值}};
    \end{tikzpicture}
    \caption{\textcolor{blue}{功耗与单帧能耗占位图}}
    \label{fig:jetson_power_placeholder}
  \end{subfigure}
  \caption{\textcolor{blue}{Jetson 平台资源评估可视化占位}}
  \label{fig:jetson_resource_placeholder}
\end{figure}

\textcolor{blue}{图~\ref{fig:jetson_resource_placeholder} 给出了 Jetson 平台资源评估的可视化占位形式。待实测数据补齐后，可分别将三种 engine 配置对应的内存占用、平均功耗和单帧能耗结果填入柱状图，以便更直观地呈现不同部署配置在平台侧资源代价上的差异。}

\subsubsection{系统展示}
系统原型围绕“双目输入--模型推理--三维姿态恢复--结果显示”四个环节组织。前端由双目相机提供同步图像，经统一预处理后送入部署模型；中间推理层负责完成特征提取、跨视图交互与姿态估计；后端输出层完成关键点三维恢复、可视化绘制与结果回显。

\textcolor{blue}{如图~\ref{fig:jetson_physical_system} 所示，当前实物系统由双目相机模组、Jetson AGX Xavier 平台及固定支撑结构组成，能够完成图像采集、模型部署与结果输出的基本流程。该实物系统图用于展示本文方法在实际硬件上的集成形态。}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{images/PhysicalSystem.jpg}
  \caption{\textcolor{blue}{基于Jetson AGX Xavier的实物系统}}
  \label{fig:jetson_physical_system}
\end{figure}

\textcolor{blue}{模型准备与转换主要在 PC 端完成，平台侧重点在于加载 TensorRT engine、接收双目输入、执行推理并输出姿态结果。当前本节仅用于说明系统组成和部署流程，不报告平台性能数值。}

\textcolor{blue}{此外，如图~\ref{fig:jetson_runtime_ui} 所示，Jetson 桌面端运行界面能够完成模型加载、推理执行与结果显示，进一步说明本文方法已经具备从部署链路到前端可视化回显的完整运行形态。该界面截图主要用于展示实际运行状态，不作为定量实验结果。}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.88\textwidth]{images/Screenshot.png}
  \caption{\textcolor{blue}{Jetson平台运行界面}}
  \label{fig:jetson_runtime_ui}
\end{figure}

\subsection{本章小结}

\textcolor{blue}{本章围绕第三章所提轻量化双目三维人体姿态估计框架的端侧落地问题，分别从已完成的 CPU 定量验证和面向 Jetson AGX Xavier 的平台验证、资源评估与系统展示三个层面展开。}
...
第二，CPU 平台上的对比实验与消融实验验证了该方法链的有效性。本文混合精度方案在 Xiaomi\,14 上以 $+2.2\,\text{mm}$ 的 MPJPE 代价实现 1.60$\times$ 推理加速，并将模型体积由 23.4\,MB 压缩至 15.2\,MB；多人场景实验进一步表明，量化后模型仍保持与第三章基线一致的时延增长规律。

\textcolor{blue}{第三，面向嵌入式 GPU 场景，本文进一步完成了基于 NVIDIA Jetson AGX Xavier 开发套件的平台验证、资源评估表位设计与系统原型展示，打通了模型导出、TensorRT engine 构建、TensorRT Runtime 装载以及双目输入与结果回显的基本流程，并明确了内存占用、平均功耗与单帧能耗的测试口径。由于当前 Jetson AGX Xavier 平台的实测结果仍在整理中，本章不对其性能作定量结论，相关实验数据将在后续补充。}
...
第一，本文提出了面向端侧多人场景的轻量化非对称双目三维人体姿态估计框架。通过“高容量主分支 + 轻量辅助分支”的非对称骨干设计、EFEM 的一维极线特征交互以及 DWT 的显式几何求解，模型在 RT-Pose 上以 11.70\,M 参数量和 13.93\,G FLOPs 实现 158.6\,mm 的平均 MPJPE，较最优级联范式降低 43.4\,mm。其结论边界是：该优势建立在立体校正有效、双目视差仍具可利用性的静态图像场景上。
...
第三，本文提出了面向目标平台的硬件感知混合精度部署方法链。通过“端到端延迟拆解 + Roofline 瓶颈分析 + 模块级量化敏感性筛查”，本文识别出骨干卷积是低风险高收益量化对象，而 EFEM 注意力分数、热力图/置信度输出与 DWT 求解链路属于高敏感保护层；在此基础上设计的分层混合精度方案在 Xiaomi 14 上将单人场景推理时延由 116.0\,ms 降低至 72.3\,ms，实现 1.60$\times$ 加速，并将模型体积由 23.4\,MB 压缩至 15.2\,MB，MPJPE 仅增加 2.2\,mm。其结论边界是：该精度分配策略当前只在 Snapdragon 8 Gen3 CPU + MNN 执行环境中完成验证，迁移到 GPU/NPU/DSP 或其他推理后端时仍需重新标定热点与敏感层。
