...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.96\textwidth]{images/CSPDarknetArch}
  \caption{非对称双目骨干网络整体结构（左）与 CSP Block 内部结构（右）。
...
在上述背景下，模型压缩与硬件感知优化成为提升端侧可用性的有效路径。其中，量化压缩通过将 FP32 浮点权重映射为低比特整数表示（如 INT8），显著降低模型存储与访存带宽需求；结合端侧硬件对 FP16 NEON 指令的支持，可充分发挥低精度计算的加速能力，从而在不改变算法整体流程的前提下改善时延与能耗表现。但同时，量化引入的离散化误差与动态范围截断会改变特征分布，可能对跨视图对齐、几何推断与深度细化等环节产生累积影响，进而造成三维重建精度下降。因此，如何在端侧约束下实现精度--时延--内存多维平衡，是本章研究的核心动机。
...
由表~\ref{tab:module_latency} 与图~\ref{fig:module_delay_pie} 可得到以下结论。骨干网络是延迟主体：左右两分支合计占端到端总延迟的绝大部分，其中左分支（全量 CSPDarknet-M）因通道宽度更大（步幅 $s\!=\!8/16/32$ 处分别为 256/512/1024 通道）而耗时显著高于右分支（128/256/512 通道）；这一分布说明骨干推理阶段是优化的首要目标。EFEM 贡献可观的额外开销：极线注意力模块在 $1/16$ 分辨率特征图上沿视差窗口执行相关运算，由于需要对左分支键值特征进行逐视差位移采样，访存模式不规则，导致其实测延迟与其理论 FLOPs 占比不成正比，是量化收益的次要来源。DWT 求解器延迟可忽略：可微加权三角化仅在 $J\!=\!17$ 个关节上执行 $4\!\times\!4$ 矩阵运算，批大小为 1 时计算量极小，在端到端延迟中占比低于 1\%，无量化必要。

\paragraph{CSPDarknet 结构特性与量化适应性分析。}
CSPDarknet 的核心是跨阶段部分网络（Cross Stage Partial，CSP）设计（网络整体结构详见第三章图~\ref{fig:cspdarknet_arch}）：在每个 CSP 块中，输入特征图沿通道维度分为两路，一路经多个 Bottleneck 残差单元进行深层特征提取，另一路直接跨阶段传播，两路最终由 $1\times1$ 卷积拼接融合。这种"部分连接"结构相比全连接残差网络可减少约 20\% 的参数量与重复梯度计算，同时保留残差连接的多尺度特征复用优势；所有非线性激活采用 SiLU（$f(x)=x\cdot\sigma(x)$），其输出分布中心稳定、动态范围有界，per-channel 对称量化步长可有效覆盖各通道的幅值差异。

从量化适应性角度看，CSPDarknet 的算子组合对端侧 INT8 部署极为友好：骨干中所有卷积层均为标准 Conv$+$BN$+$SiLU 结构，MNN 的图优化阶段可自动将 BN 参数（$\gamma, \beta, \mu, \sigma$）吸收至前置 Conv 权重，形成单一的 merged Conv 权重矩阵，量化对象由此简化为一个纯权重矩阵而非含 BN 中间激活的复合层，有利于 per-channel 静态量化精确建模每个输出通道的幅值分布。此外，不对称双分支设计（左分支 width=1.0 / depth=1.0，右分支 width=0.5 / depth=0.34）使权重量在两分支间呈现约 $4\times$ 的差距（参数量分别约 9.2\,M 与 2.0\,M），实测延迟比为 $76.7\,\text{ms}/19.4\,\text{ms}\approx3.95$，与理论计算量比高度吻合，说明骨干延迟主要由卷积算术负载决定，INT8 对计算密集型骨干的加速效果可以得到充分利用。

图~\ref{fig:cspdarknet_stage_pie} 进一步给出了双分支各阶段的延迟占比（基于 FLOPs 比例从实测总延迟推算）。两分支均呈现"中间阶段（P3/P4）权重最大"的分布规律：P3 与 P4 阶段合计占各分支延迟约 $70\%$，原因在于这两个阶段特征图分辨率（$60\!\times\!80$ 与 $30\!\times\!40$）适中而通道数（256/512）已相当可观，FLOPs 密度最高；Stem 层分辨率虽大但通道数少，延迟占比反而较低；P5 阶段通道最多（1024/512）但分辨率最小（$15\!\times\!20$），FLOPs 绝对值低于 P4。这一分布意味着 INT8 量化对 P3/P4 阶段的加速收益最为关键，且两分支同构的阶段划分使量化精度损失可在相同粒度上被敏感性分析一并覆盖。
...
上述延迟分布揭示了一个关键特征：在 batch\,=\,1 的端侧推理场景下，骨干与 EFEM 的卷积权重在每次前向中均需从主存重新加载，权重复用率极低，产生了显著的内存带宽压力。骨干大卷积虽属计算密集型，但模型总权重（FP16 约 23.4\,MB）超出骁龙 8 Gen3 的 LLC 容量（典型 8--16\,MB），每次前向推理的权重均需从 DRAM 重新加载，LLC 命中率受限；INT8 量化将权重体积压缩约 50\%（$\to$ 约 11.7\,MB），有助于提升 LLC 命中率并改善实际运行性能。EFEM 投影卷积与预测头等访存密集层则可直接受益于权重带宽减半。上述判断将在下节通过 Roofline 分析得到定量验证，并直接支撑混合精度量化方案的设计动机——通过 INT8 量化将权重访存带宽降低约 $2\times$，是在当前平台上获取实质性延迟收益的最有效路径。
...
目标平台参数。 Xiaomi\,14 搭载高通骁龙 8 Gen3，大核集群配置为 1×Cortex-X4（3.3\,GHz）与 3×Cortex-A720（3.15\,GHz）。在 NEON FP16 指令下，4 线程理论峰值算力约为 $P_{\text{peak}} \approx 100\,\text{GFLOPS}$；LPDDR5X 双通道理论带宽约为 $B_{\text{peak}} \approx 68\,\text{GB/s}$，考虑 LLC 命中率与内存控制器利用率后，有效带宽取 $B_{\text{eff}} \approx 40\,\text{GB/s}$。对应屋脊点为：
\[
...
各模块算术强度估算。 对于 $3\times3$ 卷积（$C_{\text{in}}$ 输入通道、$C_{\text{out}}$ 输出通道、分辨率 $H\!\times\! W$，FP16），其算术强度为：
\[
  \mathrm{AI}_{3\times3}
  = \frac{2\,C_{\text{in}}\cdot 9\cdot C_{\text{out}}\cdot HW}
         {2\bigl(9\,C_{\text{in}}\,C_{\text{out}}
                 ...
                 + C_{\text{out}}\,HW\bigr)}
\]
在 stride=8 输出分辨率（$60\!\times\!80$，$C\!=\!256$）处，$\mathrm{AI}\approx923$\,FLOP/byte；stride=16 处（$30\!\times\!40$，$C\!=\!512$），$\mathrm{AI}\approx784$\,FLOP/byte；stride=32 处（$15\!\times\!20$，$C\!=\!1024$），$\mathrm{AI}\approx280$\,FLOP/byte。三个尺度均远高于屋脊点，说明骨干 $3\!\times\!3$ 卷积为计算密集型，瓶颈在于 ARMv8.2 FP16 MAC 吞吐量。

对于 EFEM 中的 $1\!\times\!1$ 投影卷积（$C_{\text{in}}=512$，$C_{\text{out}}=64$，$30\!\times\!40$），$\mathrm{AI}\approx11.3$\,FLOP/byte，低于屋脊点，为访存密集型；而视差维度的 gather/correlation 操作在 $D\!=\!24$ 个位移上对 $[B, C', H, W]$ 特征图进行复制与点积，$\mathrm{AI}\approx0.04$\,FLOP/byte，属于极端访存密集型，这也解释了 EFEM 实测延迟高于其 FLOPs 占比的现象。热力图头的 $1\!\times\!1$ 卷积（$C_{\text{in}}=256$，$C_{\text{out}}=17$）$\mathrm{AI}\approx15.7$\,FLOP/byte，同样为访存密集型。表~\ref{tab:roofline_summary} 汇总了各模块的 Roofline 分析结果。
...
INT8 量化对两类瓶颈的统一收益。 INT8 量化为上述两类瓶颈提供互补的优化路径：对计算密集型骨干大卷积，骁龙 8 Gen3 CPU 的 ARMv8.2 dotprod 指令在 4 线程下的 INT8 吞吐量约为 FP16 的 $2\times$（约 200\,GOPS），可直接降低卷积执行时间；对访存密集型 EFEM 投影、头部卷积及 gather 操作，INT8 权重将每次前向所需从 DRAM 加载的权重字节数减半（FP16 的 2\,byte/param 降至 INT8 的 1\,byte/param），在有效带宽约束下内存传输时间随之减半。综合两类效应，预期端到端延迟可获得约 1.5--1.7$\times$ 的加速，同时模型体积压缩约 $2\times$，具体取决于分层量化策略中各精度层的权重占比。

\subsubsection{量化敏感性分析：几何精度高风险层识别}

精度敏感性分析的目标是在施加 INT8 量化之前，通过系统的逐组量化实验识别对三维重建精度影响最大的层，从而指导分层精度分配设计。

实验方案。 以第三章 FP16 全精度模型为基准，逐组将各模块的权重或激活替换为 INT8 量化表示（per-channel 对称量化），其余层保持 FP16，使用 RT-Pose 验证集上的 MPJPE 均值作为精度指标，记录与 FP16 基线的变化量 $\Delta\text{MPJPE}$。量化目标分为权重量化（对卷积权重矩阵的静态量化）与激活量化（对中间特征图的动态量化），两者对端侧推理精度的影响机制不同。

分析结果。 各模块量化敏感性如表~\ref{tab:quant_sensitivity} 所示。
...
结果分析。 骨干 Conv+BN 权重（左/右分支）及 EFEM 投影权重（$\varphi_q/\varphi_k/\varphi_v$）对 INT8 量化不敏感（$\Delta\text{MPJPE}<2.5\,\text{mm}$）：这些层均为标准卷积权重，激活分布范围有限，per-channel 量化可有效覆盖各输出通道的动态范围差异，量化误差主要体现为低幅值特征的轻微压缩，不影响关节热力图的峰值判别。

相对地，EFEM 注意力分数（$+6.8\,\text{mm}$）与 DWT 输入/输出（$+7.3\,\text{mm}$）显现出明显高敏感特性。对于 EFEM 注意力，视差维度的 softmax 操作对分数精度高度依赖：INT8 量化步长可能抹平精细视差位置的分数区分，导致关节对应性判断出错进而影响三维坐标。对于 DWT，$4\times4$ 矩阵特征分解对数值精度敏感，INT8 激活引入的离散化误差会直接传递至三维坐标估计，造成系统性偏差。热力图与置信度头激活的中等敏感性（$+3$--$4\,\text{mm}$）源于软 argmax 估计的二维坐标对峰值像素与邻域区分度的依赖：INT8 激活量化削弱该区分度，导致坐标出现约半像素级系统偏移，在 stride=16 倍率下对应约 $8\,\text{mm}$ 的三维误差放大。此外，MNN CPU 量化路径仅支持 FP32 与 INT8 两种精度，对量化豁免层自动以 FP32 执行，FP32 的数值精度对上述高敏感操作亦更为充裕。综合分析，本文将精度保护层界定为：EFEM 注意力分数（保留 FP32），热力图与置信度头激活（保留 FP32），DWT 三角化求解器全层（保留 FP32）；其余模块的权重施加 INT8 per-channel 量化。

\subsection{混合精度量化方案设计}

针对上节识别的模块特性与精度风险，本节提出面向本模型的混合精度量化方案。总体策略为：对访存密集的骨干网络与 EFEM 特征提取部分采用 INT8 per-channel 后训练量化以获得最大访存收益；对几何精度敏感的极线注意力分数、热力图头与三角化求解器统一保留 FP32，以规避量化误差对跨视图匹配与三维重建的累积影响。本文采用后训练量化（PTQ）而非量化感知训练（QAT）：由于混合精度设计已将精度高风险层隔离在高精度区间，剩余 INT8 化部分均为标准 Conv+BN 结构，PTQ 精度损失预期可控，无需引入 QAT 的额外训练代价。部署链路方面，以 PyTorch 训练模型为起点，经 ONNX 导出、Conv-BN 融合等图优化步骤后通过 MNN 完成端侧编译；DWT 中的特征值分解算子保留在 CPU FP32 执行，其计算开销可忽略不计。

\subsubsection{分层精度分配设计}

基于量化敏感性分析，本节提出分层混合精度量化方案，核心原则为：对访存/计算密集且量化不敏感的卷积权重施加 INT8 per-channel 量化；对几何精度高风险的激活和求解器统一保留 FP32，在端侧效率与三维重建精度之间取得最优平衡。需要说明的是，MNN CPU 量化路径仅支持 FP32 与 INT8 两种精度，豁免层自动以 FP32 执行，这与本文的精度保护策略天然一致。表~\ref{tab:precision_allocation} 给出详细的分层精度分配方案。
...
在上述方案中，INT8 量化覆盖的参数为：左/右骨干全部卷积权重（共约 11.1\,M 参数）与 EFEM 投影及聚合卷积权重（约 0.5\,M 参数），合计约 11.6\,M 参数，占模型总参数的 99.1\%。保留 FP32 的部分为 EFEM 注意力分数激活、热力图与置信度头（约 0.1\,M 权重参数）以及 DWT（无可训练权重），其在模型存储中占比极小但对三维精度贡献显著。

通过上述精度分配，模型权重文件的理论大小由 FP16 的 $11.70\,\text{M}\times 2\,\text{byte}\approx23.4\,\text{MB}$ 降至混合精度的约 $11.6\,\text{M}\times1\,\text{byte}+0.1\,\text{M}\times4\,\text{byte}\approx12.0\,\text{MB}$，模型体积压缩约 $48.7\%$，接近理论上限的 $2\times$。实际 MNN 格式文件尺寸约为 15.2\,MB，略高于理论权重大小（12.0\,MB），差异约 3.2\,MB 来源于 per-channel 量化缩放因子（FP32 存储，约数十 KB）、MNN protobuf 模型描述结构开销以及 Conv-BN 融合后偏置项的存储。

\paragraph{量化粒度与对称性的技术选型依据。}
本文选择 per-channel 对称 INT8 量化，而非 per-tensor 量化或非对称量化，原因如下。第一，per-channel vs per-tensor：骨干网络各输出通道在经过 BN 归一化后仍存在通道间幅值差异，在 stride=32 处（$C=1024$）尤为明显——部分通道权重集中在 $[-0.05, 0.05]$，另一些通道峰值可达 $\pm0.3$，若采用单一 per-tensor 缩放因子则前者量化步长过大（SNR 不足）、后者截断过多，per-channel 为每个输出通道独立分配缩放因子 $\text{scale}_c = \max(|W_c|)/127$，可有效适配通道间动态范围差异，精度损失更小。第二，对称量化 vs 非对称量化：对称量化零点固定为 0，骁龙 8 Gen3 的 ARMv8.2 dotprod 指令（\texttt{sdot/udot}）在对称 INT8 下无需在计算路径中叠加零点偏置修正，每次乘加操作节省一次整数加法，对计算密集型骨干层有实际吞吐提升。

\paragraph{仅量化权重（Weight-Only）而非激活量化的选择依据。}
本文通过 \texttt{--weightQuantBits 8} 仅对权重执行静态量化，激活保持 FP32 路径运行，而未对骨干激活执行 INT8 动态量化。原因有三：其一，精度保障。Conv+BN+SiLU 激活分布随输入图像内容动态变化，不同场景下激活的动态范围差异较大，静态校准数据集难以覆盖所有分布，动态量化又引入逐帧范围统计开销；相比之下，权重为训练后固定张量，per-channel 统计一次即可，精度损失可预期。其二，速度收益来源分析。由 Roofline 分析（见第~\ref{subsubsec:roofline}节）可知，骨干大卷积的延迟主要受 LLC 未命中引起的 DRAM 带宽约束：INT8 权重将骨干权重体积压缩约 $2\times$（$23.4\,\text{MB}\to11.7\,\text{MB}$），有效提升 LLC 命中率，实现约 1.60$\times$ 的实测加速；激活量化额外带来的吞吐提升在本方案的精度代价约束下性价比有限。其三，工程简洁性。Weight-only 量化无需准备校准数据集，直接基于权重统计量完成，转换步骤更简洁、复现性更强。

\subsubsection{后训练量化实施}

本文采用后训练量化（PTQ）方式，在不重新训练模型的前提下对第三章所训练的 FP16 权重进行量化压缩。

\paragraph{PTQ 充分性论证。}
选择 PTQ 而非量化感知训练（QAT）的核心依据在于：本模型 INT8 目标层（骨干 Conv+BN 权重、EFEM 投影/聚合权重）均为标准卷积结构，经 BN 归一化后权重幅值集中（$|W_c| < 3\sigma$），per-channel 缩放因子可精确覆盖权重分布，量化噪声信噪比（SNR）可达 40\,dB 以上；而几何精度高风险的激活层（EFEM 注意力分数、热力图/置信度头、DWT 输入输出）已通过精度保护策略全部豁免量化，不存在影响三维精度的激活量化误差传播链路。在此条件下，PTQ 的精度损失与 QAT 收敛后的差距极小（通常 $<1\,\text{mm}$ MPJPE），无需承担 QAT 所需的重新训练成本（数十 GPU·小时）与超参数调优复杂度。整体部署流程如图~\ref{fig:ptq_pipeline} 所示。
...
（2）逐层精度配置。 依据表~\ref{tab:precision_allocation}，在调用 \texttt{mnnconvert} 时通过 \texttt{--weightQuantBits 8}（per-channel 对称量化，\texttt{--weightQuantBlock 0}）对骨干与 EFEM 投影/聚合权重执行静态量化；对 EFEM 注意力分数激活及热力图/置信度头的所有张量，通过配置量化豁免列表保留 FP32 计算路径（MNN CPU 量化后端不支持 FP16 激活，豁免层自动回退至 FP32 执行）。

（3）量化参数确定。 权重静态量化的缩放因子由权重张量本身的 per-channel 统计量直接确定：$\text{scale}_c = \max(|W_c|)/127$，无需额外校准数据集；对豁免 INT8 的激活层，MNN CPU 量化后端自动以 FP32 路径执行，不涉及动态范围校准。DWT 中的特征值分解在 MNN 推理时通过 CPU FP32 路径执行，计算开销不超过端到端延迟的 1\%。

（4）模型打包与验证。 转换完成后，输出 \texttt{model\_mixed.mnn} 文件，通过 MNN Python API 验证输入输出形状，并在 Xiaomi\,14 真机上加载执行单次前向确认运行正确性，再进行 100 次重复推理取均值作为延迟报告值。最终得到的混合精度模型文件大小约为 $15.2\,\text{MB}$，高于纯 INT8 方案（$11.7\,\text{MB}$）约 $30\%$，但相比 FP16 基线（$23.4\,\text{MB}$）减小约 $35\%$。

\subsection{实验评估}

本节通过三组实验对所提量化方案进行系统评估。（1）量化方案对比：在相同硬件平台下对比四种精度配置（FP32 全精度、FP16 全精度、激进 INT8、本文混合精度），从精度与效率两个维度评估各方案的综合表现；（2）消融实验：逐一将混合精度方案中的 FP32 保护层替换为 INT8，定量验证各精度敏感层保留高精度的必要性；（3）多人场景量化稳定性分析：复现第三章的人数 1--10 延迟曲线，对比量化前后的端到端延迟变化，验证量化在多人动态场景下的稳定性。

\subsubsection{实验设置}

硬件平台与推理环境。 实验在 Xiaomi\,14 真机（Snapdragon 8 Gen3，16\,GB LPDDR5X）上进行，推理框架采用 MNN 2.9.0，编译时启用 ARMv8.2 dotprod（INT8 乘加加速）与 FP16 NEON 指令支持，线程数固定为 4（使用 1×Cortex-X4 + 3×Cortex-A720 大核）。推理时通过 \texttt{taskset} 将进程绑定至大核集群以消除调度抖动；每种配置预热 10 次后正式计时 100 次，取均值作为延迟报告值。批大小固定为 1，符合端侧在线推理的实际场景。

对比配置。 本节共测试四种精度配置：（1）FP32 全精度，模型以 FP32 路径执行，作为精度上界与速度下界；（2）FP16 全精度（第三章基线），全部以 FP16 路径执行；（3）激进 INT8，骨干、EFEM、热力图/置信度头全部权重及激活均量化为 INT8，DWT 保留 FP32；（4）本文混合精度，依据表~\ref{tab:precision_allocation} 分层配置。所有配置使用相同的 ONNX 图结构，仅精度配置不同；本文采用 \texttt{--weightQuantBits 8} 进行权重静态量化，缩放因子由各层权重的 per-channel 统计量直接确定，无需额外校准数据集。
...
实验结果呈现以下规律。FP32 基线延迟（208.7\,ms）约为 FP16 的 1.8$\times$，反映了 FP16 NEON 指令相对 FP32 的计算吞吐优势；两者 MPJPE 完全一致，确认 FP32 精度对该任务充分，FP16 已无精度损失。激进 INT8 实现了最大延迟压缩（1.71$\times$），但 MPJPE 增量达 $+13.4\,\text{mm}$（相对增加 8.5\%），主要源于 EFEM 注意力分数与热力图激活量化后的视差匹配失准与坐标回归偏移，验证了量化敏感性分析的预判。本文混合精度以仅 4.5\,ms 的延迟代价（72.3 vs.\ 67.8\,ms）将 MPJPE 增量从 $+13.4\,\text{mm}$ 压缩至 $+2.2\,\text{mm}$，在绝对精度（160.8\,mm）上与 FP16 基线（158.6\,mm）的差距仅为 1.4\%，同时加速比高达 1.60$\times$，模型体积降至 15.2\,MB，综合表现最优。
...
三项保护层均体现出明显的精度贡献。EFEM 注意力分数保留 FP32（抑制 $\Delta\text{MPJPE}$ 增加 6.5\,mm）是贡献最大的单因素：softmax 输入的精度损失会模糊相邻视差的匹配质量排序，该错误在三维三角化中被进一步放大。DWT 保留 FP32（$-7.4\,\text{mm}$）同样关键：$4\times4$ 矩阵特征分解对激活精度的依赖远高于卷积层，INT8 激活的量化步长会直接造成三维坐标的系统性偏差。热力图/置信度头保留 FP32（$-2.7\,\text{mm}$）贡献适中，但因头部权重参数量极少（$<0.04\,\text{M}$），改为 INT8 带来的模型压缩收益不足 0.1\,MB，精度代价不合算。三项保护层合并改为 INT8 后，MPJPE 增量（$+11.6\,\text{mm}$）已接近激进 INT8 方案（$+13.4\,\text{mm}$），说明三类精度保护层的联合贡献具有正向叠加效应而非相互抵消。此外，消融实验末行（172.4\,mm）与表~\ref{tab:quant_comparison} 中激进 INT8（172.0\,mm）存在 0.4\,mm 的微小差异：前者以本文混合精度模型为起点、仅对权重执行 per-channel 静态量化；后者在激进 INT8 配置下对激活亦施加动态量化并进行了独立推理，校准路径的细微差异导致最终误差略有出入，不影响结论的实质性解读。
...
在性能瓶颈分析方面，通过在 Xiaomi\,14（MNN/FP16/4线程）上对各模块逐一计时，确认左/右骨干网络合计占端到端延迟约 82.8\%，为优化首要目标；EFEM 的极线 gather 操作因视差窗口滑动引起的不规则访存，导致延迟与 FLOPs 占比不成正比。Roofline 分析进一步将骨干大卷积（AI = 280--923\,FLOP/byte）定性为计算密集型，而 EFEM 投影与头部 $1\times1$ 卷积（AI = 11--16\,FLOP/byte）及 gather 操作（AI $\approx 0.04$\,FLOP/byte）定性为访存密集型，揭示了 INT8 量化同时受益于两类瓶颈的理论依据。

在量化方案设计方面，敏感性分析区分出量化不敏感层（骨干权重、EFEM 投影权重，$\Delta\text{MPJPE}<2.5\,\text{mm}$）与量化高风险层（EFEM 注意力激活 $+6.8\,\text{mm}$、热力图/置信度头激活 $+3$--$4\,\text{mm}$、DWT 输入输出 $+7.3\,\text{mm}$），在此基础上提出分层混合精度量化方案：对约 99.1\% 的参数执行 INT8 per-channel 静态权重量化，对剩余精度高风险层保留 FP32 激活路径（MNN CPU 量化路径不支持 FP16 激活，豁免层统一以 FP32 执行）。方案采用后训练量化（PTQ）实施，无需重新训练，通过 PyTorch $\to$ ONNX $\to$ MNN（\texttt{mnnconvert}）的标准部署链路完成端到端编译。

在实验评估方面，本文混合精度方案在 RT-Pose 验证集上以 $+2.2\,\text{mm}$ 的 MPJPE 代价实现 1.60$\times$ 推理加速（72.3\,ms vs.\ 116.0\,ms），模型体积由 23.4\,MB 压缩至 15.2\,MB（减少 35.0\%）；相较于激进 INT8 方案，在仅损失 0.67\% 加速比的情况下将 MPJPE 增量从 $+13.4\,\text{mm}$ 压缩至 $+2.2\,\text{mm}$。消融实验逐一验证了三类保护层（EFEM 注意力、热力图/置信度头、DWT 求解器）的精度贡献，证明了分层精度设计的合理性。多人场景实验进一步证明量化不影响自底向上推理链路的延迟稳定性，1.60$\times$ 加速比在 $M=1$--$10$ 人范围内保持不变，验证了本文方案在多人动态场景下的部署稳定性与实用性。
