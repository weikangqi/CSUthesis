...
Zhang 和 Guo 提出 DMCPose\citess{zhang2024dmcpose}，以前端高精度二维姿态网络提取双目关键点观测，再结合关键点级视差、多视图一致性与注意力机制进行动态融合，从而绕开传统稠密立体匹配的整图搜索过程。该方法说明双目 3D 人体姿态估计可以围绕人体关节建立更轻量的几何恢复路径，是近年来“双目几何先验 + 注意力融合”思路的代表性工作之一；但其验证场景仍以受控数据集上的单人动作为主，尚未进一步覆盖多人实时场景与资源受限端侧部署条件。
...
第三章为基于双目相机的轻量化3D人体姿态检测网络设计。提出一种轻量化非对称双目学习框架：以非对称骨干网络消除左右对称冗余，以极线特征增强模块（EFEM）将跨视图匹配限制在一维极线搜索空间，以可微加权三角化姿态求解器（DWT）实现端到端的三维关节坐标恢复，并通过对比实验与消融实验验证各模块的有效性，如图~\ref{fig:system_overview}所示。

第四章为硬件感知部署与量化压缩。以 Snapdragon 8 Gen3 CPU 为已完成定量验证的平台，通过延迟拆解与 Roofline 模型定位推理瓶颈，借助量化敏感性分析制定分层混合精度量化方案，经后训练量化（PTQ）流程输出可部署的 MNN 模型，并在真机上验证精度保持与推理加速效果；此外，本章还给出面向华为昇腾 Atlas 200I 平台的平台验证与系统展示，为后续补充嵌入式 NPU 数据做好准备。
...
面向移动端或嵌入式设备的视觉算法受到算力、内存带宽与功耗的严格约束。对于双目3D人体姿态估计而言，网络需在参数量、计算量（FLOPs）、峰值内存与精度-效率折中四个维度协同权衡，同时避免传统立体匹配中高维代价体（Cost Volume）带来的显存开销。国内关于轻量级神经网络架构的高水平综述指出，端侧模型设计不能只关注参数量缩减，还需要兼顾算子友好性与实际部署效率\citess{zhu2020lightweightnncn}。关于深度神经网络压缩与加速的系统研究进一步表明，端侧优化还应同时考虑访存压力与硬件执行特性\citess{ji2018compressioncn}。深度网络模型压缩综述则从压缩策略适用边界角度强调，结构设计与压缩策略需要协同考虑\citess{gao2021deepcompresscn}。轻量化设计通常遵循"减少高维张量、优先2D算子、控制通道数与分辨率"的总体原则。本小节从结构轻量化设计、模型压缩与端侧部署工程优化三个层次依次介绍在边缘端实现高效推理的常用方法。
...
本章系统梳理了本文研究所依赖的核心理论与技术基础。在深度学习方面，介绍了卷积神经网络的层次特征提取机制，以及 Transformer 的多头注意力与交叉注意力框架，它们分别适用于高效视觉特征提取和跨位置、跨视图的全局关联建模。在人体姿态估计方面，归纳了2D/3D姿态估计的主流范式、热力图与坐标回归两类表示方法，以及二维与三维姿态估计的常用评价指标，明确了双目3D姿态估计的任务定义与评测体系。在双目立体视觉方面，阐述了针孔成像模型、极线几何约束与三角测量深度恢复方法，为高效跨视图特征匹配与三维坐标恢复奠定了几何基础。在轻量化网络设计方面，总结了深度可分离卷积、倒残差结构等高效骨干范式，以及量化、剪枝与知识蒸馏等模型压缩策略，为面向资源受限设备的视觉算法设计提供了理论参照。
...
\subsection{研究背景}
在上一章对双目三维姿态估计与端侧轻量化技术的研究现状进行梳理后可以发现，三维人体姿态估计的研究重点正从单纯追求离线精度，逐步转向兼顾实时性、能效与可部署性的综合优化。在物联网与边缘计算驱动的应用场景中，系统通常需要在移动端或嵌入式设备上持续运行，例如服务机器人、人机交互终端、可穿戴设备与智能摄像头等\citess{yuan2024aiotinfercn}。此类平台普遍受到算力、内存与功耗等资源约束，同时还面临隐私保护与网络不确定性等现实因素；边缘智能协同推理研究也表明，端侧系统能否落地往往取决于任务划分、访存模式与硬件友好性等多种工程条件\citess{wang2023edgecollabcn}。因此，能够在端侧稳定运行的三维姿态估计方法必须在精度与效率之间建立可解释且可复现的权衡，而这也构成了本章研究的直接背景。
...
然而，双目约束并不天然等价于端侧友好。现有许多双目深度或三维重建方法仍依赖高维代价体构建与密集正则化，往往需要三维卷积或大规模迭代优化才能获得较高精度，这会带来显存占用高、访存压力大与推理延迟不可控等问题\citess{chang2018pyramid}。面向移动平台的立体匹配研究则进一步表明，为获得更可控的部署成本，必须主动压缩跨视图交互与代价聚合链路\citess{shamsafar2022mobilestereonet}。另一方面，若直接采用左右对称的双分支结构分别完成特征提取与姿态预测，骨干网络与多尺度融合模块的计算量将近似倍增，导致大量跨视图冗余计算；在多人场景中，该冗余会随实例数增加而放大，进一步削弱端侧实时性，并使系统时延随场景复杂度产生明显波动。
...
针对 3.2 节归纳的两类瓶颈——稠密级联开销（问题A）与左右对称冗余（问题B）——本节提出轻量化非对称双目学习框架。对于双目三维人体姿态估计而言，任务本质上是围绕同一组人体关节完成二维定位、跨视图对应与三维恢复，而不是让左右视图分别承担一套完全对等的重型三维理解流程。基于这一点，本文采用“主分支负责稳定语义与几何参照、辅助分支负责提供跨视图匹配线索”的非对称分工，将有限计算资源集中到对三维恢复更关键的表征上。
...
\subsubsection{实验结果} 
\paragraph{对比协议}
本文将对比方法划分为两类：一类是仅使用左视图$I_l$作为输入的单目方法，包括 Simple Baseline\citess{martinez2017simple}、JointFormer\citess{lutz2022jointformer} 与 RTMW3D\citess{jiang2024rtmw}；另一类是使用双目输入$(I_l,I_r)$、遵循“稠密深度先验 + 关节回代”流程的级联范式，包括 MobiDepthPose\citess{zhang2022mobidepth}、RTMO\citess{lu2024rtmo}+SGBM 与 RTMO\citess{lu2024rtmo}+MobileStereoNet\citess{shamsafar2022mobilestereonet}。所有对比方法均在相同数据划分下微调与评测，其中 RTMW3D 主要作为相同划分下的强表征参考，用于说明本文方法在端侧可部署约束下的精度--效率权衡。
...
表~\ref{tab:ablation_efem} 对应的问题是“轻量右分支如何在不恢复稠密代价体的前提下获得稳定几何线索”。从结果看，拼接+$1\times1$卷积虽能带来明显收益，但由于缺少显式几何约束，仍可能引入噪声匹配；EFEM 则在极线约束下将跨视图交互限制在一维视差窗口内，使匹配更具几何可解释性，从而进一步降低误差。“无跨视图交互”设置下虽然仍保留双目输入，但右视图轻量分支在遮挡与弱纹理场景下更易出现关键点多峰与定位抖动，导致视差误差在远距离小视差区域被放大；也就是说，此时非对称结构中的轻量右分支难以单独支撑稳定的跨视图几何约束。EFEM 通过沿极线引入低开销的跨视图一致性信息，可显著提升右分支观测的稳定性，从而释放双目几何在该数据集上的优势；其收益成立的前提仍是立体校正残差受控且$D_{\max}$能够覆盖有效视差范围。
...
除高性能手机 CPU 外，嵌入式开发板上的 NPU 也是边缘落地的重要载体。这类平台通常具有更好的能效比和更灵活的本地接入方式，适合部署在机器人终端、固定式感知节点与轻量边缘设备之中。然而，NPU 友好并不等价于“模型可直接迁移”：算子白名单、子图切分、低比特量化兼容性以及 NPU/CPU 异构回退都会影响最终部署效果。基于这一考虑，本文在完成 Snapdragon 8 Gen3 CPU 平台上的量化验证之外，进一步给出面向华为昇腾 Atlas 200I 平台的平台验证与系统原型设计，以便后续围绕嵌入式 NPU 的实测指标开展补充评估。
...
除上述 CPU + MNN 部署链路外，本文还同步设计了面向华为昇腾 Atlas 200I 平台的扩展路径，即 `PyTorch $\rightarrow$ ONNX $\rightarrow$ \texttt{ATC} 模型转换 $\rightarrow$ OM 离线模型 $\rightarrow$ \texttt{AscendCL} Runtime 装载 $\rightarrow$ Atlas 200I 平台侧部署`。其中，主体卷积层优先映射至 NPU 执行，高敏感或暂不支持的算子则预留异构回退接口。需要说明的是，昇腾 Atlas 200I 部署部分当前重点在于完成链路设计与系统接入，相关性能与精度评估结果将在后续实验中补充。

\subsection{实验评估}

本节首先给出已完成的 CPU 平台定量实验结果，包括不同精度配置下的精度--效率权衡、精度敏感层消融以及多人场景下的时延稳定性分析；随后补充面向华为昇腾 Atlas 200I 平台的部署验证与系统展示。由于昇腾 Atlas 200I 平台的实测数据仍在补充中，相关内容当前以部署链路打通和系统原型展示为主，不对平台性能作定量结论。

\subsubsection{实验设置}

硬件平台与推理环境。已完成的定量实验均在 Xiaomi\,14 真机（Snapdragon 8 Gen3，16\,GB LPDDR5X）上进行，推理框架采用 MNN 2.9.0，启用 ARMv8.2 dotprod 与 FP16 NEON 支持。所有实验均固定 4 线程 CPU 执行环境，预热 10 次后正式计时 100 次，取均值作为延迟报告值；批大小固定为 1，输入分辨率统一为 $640\times480$。面向华为昇腾 Atlas 200I 平台的验证不纳入本节定量对比，其部署环境与系统展示将在 4.4.5 节和 4.4.6 节给出。

对比配置。本节测试四种精度配置：FP32 全精度、FP16 全精度（第三章基线）、激进 INT8，以及本文混合精度。四种配置共享相同的网络结构、输入尺寸与推理协议，仅精度设置不同。

评估指标。精度以 RT-Pose 验证集上的 MPJPE 均值（mm）衡量，同时记录与 FP16 基线的偏差 $\Delta\text{MPJPE}$；效率以端到端推理延迟（ms，单人场景）和模型文件大小（MB）表征。对于 4.4.5 节的昇腾 Atlas 200I 平台验证，后续将沿用相同的精度指标补充跨平台结果。
...
\subsubsection{昇腾Atlas 200I平台验证}
为验证所提方法在嵌入式 NPU 平台上的可部署性，本文选取华为昇腾 Atlas 200I 开发板作为验证对象，重点完成模型转换、运行时接入与基本推理链路打通；其验证环境与软件配置如表~\ref{tab:ascend_env} 所示。

\begin{table}[htbp]
  \centering
  \small
\caption{Atlas 200I验证环境与软件配置}
  \label{tab:ascend_env}
  \begin{tabular}{@{}p{0.18\textwidth}p{0.26\textwidth}p{0.42\textwidth}@{}}
    \toprule
    项目 & 配置 & 说明 \\
    \midrule
    验证平台 & 华为昇腾 Atlas 200I 开发板 & 作为嵌入式 NPU 验证对象 \\
    模型输入 & $640\times480$，batch=1 & 与 CPU 平台保持一致 \\
    模型来源 & ONNX 导出模型 & 由 PyTorch 模型导出后供昇腾工具链转换 \\
    转换工具 & CANN / ATC & 负责模型转换与编译 \\
    运行时 & AscendCL Runtime & 负责平台侧装载与执行 \\
    精度配置 & INT8 / 混合精度 & 以平台侧实际可执行配置为准 \\
    运行方式 & NPU 主路径 + 必要时 CPU 回退 & 为不支持算子预留异构执行接口 \\
    \bottomrule
  \end{tabular}
\end{table}

验证采用 `PyTorch $\rightarrow$ ONNX $\rightarrow$ \texttt{ATC} $\rightarrow$ OM $\rightarrow$ \texttt{AscendCL} Runtime $\rightarrow$ Atlas 200I` 的部署链路，在平台侧完成双目图像输入、模型调用与结果回传。当前该部分仅用于说明部署可行性，不展开跨平台性能分析，相关时延与精度数据将在后续补充。

\subsubsection{系统展示}
系统原型围绕“双目输入--模型推理--三维姿态恢复--结果显示”四个环节组织。前端由双目相机提供同步图像，经统一预处理后送入部署模型；中间推理层负责完成特征提取、跨视图交互与姿态估计；后端输出层完成关键点三维恢复、可视化绘制与结果回显。

如图~\ref{fig:ascend_physical_system} 所示，当前实物系统由双目相机模组、昇腾 Atlas 200I 平台及固定支撑结构组成，能够完成图像采集、模型部署与结果输出的基本流程。该实物系统图用于展示本文方法在实际硬件上的集成形态。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.80\textwidth]{images/PhysicalSystem.jpg}
  \caption{基于Atlas 200I的实物系统}
  \label{fig:ascend_physical_system}
\end{figure}

模型准备与转换主要在 PC 端完成，平台侧重点在于加载 OM 模型、接收双目输入、执行推理并输出姿态结果。当前本节仅用于说明系统组成和部署流程，不报告平台性能数值。

\subsection{本章小结}

本章围绕第三章所提轻量化双目三维人体姿态估计框架的端侧落地问题，分别从已完成的 CPU 定量验证和面向昇腾 Atlas 200I 的平台验证与系统展示两个层面展开。
...
第三，面向嵌入式 NPU 场景，本文进一步完成了基于华为昇腾 Atlas 200I 的平台验证与系统原型展示，打通了模型导出、ATC 转换、AscendCL 运行时装载以及双目输入与结果回显的基本流程。由于当前昇腾 Atlas 200I 平台的实测结果仍在整理中，本章不对其性能作定量结论，相关实验数据将在后续补充。
