...
（1）构建轻量化双目3D人体姿态估计框架，研究左右视图特征提取、跨视图信息交互与三维关节点回归的整体流程。

（2）设计面向端侧部署的高效网络结构，在保证姿态估计精度的前提下，降低参数量、计算量与峰值内存占用，提高实时性与能效比。

（3）探索知识蒸馏、量化等模型压缩策略及其与网络结构轻量化的协同方式，分析不同压缩方案对精度与效率的影响。

（4）基于复杂真实场景数据集（如RT-Pose）开展实验评估，验证所提方法在弱纹理与光照变化等条件下的鲁棒性与泛化能力。
\subsection{论文组织结构}
...
（1）在跨视图匹配环节，引入极线特征增强模块：利用极线约束将跨视图匹配从二维搜索降为一维视差窗口搜索，使特征交互聚焦于与关键点相关的几何线索，从而避免显式构建全图稠密深度与高维代价体，大幅削减无效像素的计算开销（见第~\ref{subsubsec:efem}节）。

（2）在三维求解环节，设计可微加权三角化姿态求解器：直接将双目二维观测与相机模型融合为三维关节坐标，绕过级联深度估计的多阶段误差传递，并使 MPJPE 监督信号可端到端回传，消除中间监督与最终目标之间的不一致（见第~\ref{subsubsec:dwt}节与第~\ref{subsubsec:align_loss}节）。

针对问题B（左右对称双分支结构的跨视图冗余计算），本章从骨干架构与训练策略两个层面协同解决：

（1） 在骨干架构上，采用非对称双目骨干网络（左重右轻）：左分支保持全量容量以提供高质量的几何与语义表征，右分支采用轻量结构以大幅降低冗余计算，从而打破传统双全量骨干近似翻倍的计算瓶颈（见第~\ref{subsubsec:asym_backbone}节）。

（2）在训练策略上，引入跨分支表征一致性约束：弥补非对称设计带来的左右分支表征差异，使轻量右分支在训练中获得更强的几何感知能力，且该约束仅作用于训练阶段，推理时不增加额外开销（见第~\ref{subsubsec:align_loss}节）。
...
（1）一侧关键点热力图峰值尖锐、另一侧发散或多峰；

（2）一侧发生人体自遮挡、出框或强反光导致误检；

（3）远距离小视差区域对像素误差更敏感，单侧抖动会被三角测量放大。
...
从端侧部署角度看，移动端 CPU 与边缘设备通常同时受到算力、内存带宽、缓存容量与功耗预算的约束，模型能否落地不仅取决于参数规模，还取决于权重访存模式、算子调度方式以及中间激活的存取开销。现有工作通常将结构裁剪、算子优化与量化部署协同设计\cite{jacob2018quantization,banner2019post,nagel2019data,xiao2023smoothquant}；这也说明，端侧量化并非“全模型一刀切”即可达到最优效果，精度风险往往集中在激活分布不稳定层、归一化与匹配相关环节，以及直接参与几何恢复的求解环节。对于本文同时包含卷积主干、跨视图匹配、softmax 与几何求解的模型，更合适的路径是先识别端侧瓶颈，再确定哪些层适合量化、哪些层必须保护。

\subsection{问题分析}

本节围绕“哪些模块最值得量化、哪些模块不能轻易量化”这一核心问题展开。具体分为三步：先通过实测延迟拆解回答“优化重点在哪里”，再借助 Roofline 模型回答“热点为何成为瓶颈”，最后通过量化敏感性分析回答“哪些层可以量化、哪些层必须保护”。

\subsubsection{端侧推理瓶颈的实测分析}

4.2.1 先回答“端侧推理时间主要耗在哪些模块上”。为定位第三章所提框架在端侧的计算热点，本节以 Xiaomi 14 为目标平台，在 MNN、FP16、ARMv8.2、4 线程、输入 $640\times480$、batch=1、单人场景下对各模块逐一计时。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.84\textwidth]{images/ModuleDelayRatioFanChart}
  \caption{各模块端到端延迟占比}
...
在确认“骨干是主热点、EFEM 是次级热点”之后，本节进一步回答“这些热点为何形成瓶颈，以及量化收益主要来自哪里”。为此，引入 Roofline 模型以算术强度（Arithmetic Intensity, AI）刻画算子“算得多还是搬得多”，其中
...
4.2.3 最后回答“哪些热点可以量化、哪些关键环节必须保护”。本节以第三章 FP16 模型为基线，在 RT-Pose 验证集上采用逐组替换方式评估各模块的 INT8 敏感性：每次仅将某一模块的权重或激活替换为 INT8，其余模块保持 FP16，并记录相对基线的 $\Delta\text{MPJPE}$。
...
总体而言，本文工作的意义主要体现在两个方面：一方面，从模型设计角度提出了适合移动端双目多人三维姿态估计的轻量化网络结构，降低了传统双目方法对对称大骨干和稠密代价体的依赖；另一方面，从部署优化角度形成了面向目标平台的硬件感知量化分析与落地方法，为后续同类端侧视觉模型的压缩部署提供了可借鉴的研究思路。
